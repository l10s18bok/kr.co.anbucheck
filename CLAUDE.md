...
| 3차 (안전망)   | `lib/app/core/services/local_alarm_service.dart`                                                  | 일일 안부 확인 안전망 로컬 알림 (양 플랫폼 매일 반복, daily repeat via `matchDateTimeComponents.time`). **iOS**: heartbeat 예약 시각 정시에 fire — BGTaskScheduler 미사용으로 사실상 PRIMARY 트리거. **Android**: heartbeat 예약 시각 + 3시간에 fire — WorkManager one-off + periodic 15분 + 앱 열기(2차)가 모두 실패해 worker 자체가 OEM/사용자에 의해 cancel된 케이스를 메우는 LAST-RESORT 안전망. 자정 넘기면 자연 롤오버(예: 22:30 → 다음 날 01:30). 알림 탭은 앱을 **safety_home으로 전환**하고 홈 컨트롤러의 `onInit`/`onResumed`가 미전송 heartbeat 자동 전송. heartbeat 성공 시 `_onHeartbeatSent`가 `schedule(forceNextDay: true)` 호출 → `cancel(_alarmId)`로 예약 + 표시 중 알림을 모두 제거하고 내일자로 재예약 — stale 알림 잔존 없음. **로컬 알림 탭 라우팅**: iOS `gs_deadman` / Android `safety_net`·`send_failed` 모두 **safety_home으로 이동**(`fcm_service._routeToSafetyHome` — 역할 인식: G+S는 Dashboard base + push, 순수 S는 단독; kill 런치는 splash가 처리). FCM 푸시(`alert_*` 등)는 알림 목록으로 가는 것과 구분됨. retry 3회 실패 알림(`_sendFailedId`)은 `cancelSendFailed()`로 별도 정리                                                                                                                                                                                                |
