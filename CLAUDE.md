...
| 공유 캐시      | `lib/app/core/services/guardian_subject_service.dart`                                             | 보호자 대상자 목록 공유 캐시 (2분 TTL). 대시보드·설정·연결관리에서 동일 데이터 사용. 구독 상태 동기화. **대상자 별칭 서버 동기화(`syncAliasesIfChanged`)** — 별칭은 로컬(`NicknameLocalDatasource`)이 원본이지만, 보호자 Push 제목에 "누구의 알림인지"(`⚠ 주의 · 삼촌`)를 표시하려면 앱이 꺼져 있어도 OS가 그리는 제목을 서버가 만들어야 하므로 사본을 `guardians.alias`에 올린다(`PUT /api/v1/subjects/aliases`). 트리거는 **스냅샷 비교 하나**(`nicknames_synced`와 다를 때만 전송, 성공 시에만 스냅샷 갱신) — 백필·개별 저장·실패 재시도가 이 하나로 전부 처리된다. 일회성 플래그로 바꾸면 실패가 영구화되므로 금지. 호출은 `load()` 성공 직후 + 연결관리 별칭 수정 직후 2곳, 둘 다 `unawaited` **fire-and-forget**(동기화 실패가 연결·저장·화면을 막으면 안 됨). 상세는 PRD-FrontEnd §9.6.1 |
...
⚠️ **FAQ·사용설명·홈(폰 목업 4곳) 세 곳이 이 저장소의 번역 파일을 읽어 만들어진다.** `lib/app/core/translations/*.dart`의 **101개 키**를 **이름 변경·삭제하면 페이지 빌드가 실패**한다(`extract_strings.py`가 exit 1). 문구만 바꾸는 것은 안전하며, averic-lab에서 `extract_strings.py`를 재실행하면 20개 언어에 반영된다.
...
- **홈 「최대 5명」 연결·대시보드 목업**(2026-08-08 추가)이 쓰는 것: `connection_title`·`connection_connected_subjects`·`connection_managed_count_value`(@current/@max)·`connection_heartbeat_schedule`(@time) 4개가 이때 새로 추가됐고, 나머지(`add_subject_*` 입력 화면, `guardian_*` 대시보드, `nav_*`)는 사용설명·히어로와 겹친다. ⚠️ 이 섹션의 대시보드 카드는 **전부 정상(안전) 등급**이다 — 주제가 인원 한도이지 경고 등급이 아니며, 등급은 「단계별 신호」 섹션이 따로 맡는다
- 목업 카드에 보이는 **대상자 별칭**(`엄마`/`Mom`/`Мама`…)은 이 저장소의 키가 아니다 — 사용자가 직접 입력하는 값이라 앱 번역 파일에 없고, averic-lab `i18n/translations.json`의 `nick1`~`nick4`가 단일 출처다. 여기서 찾지 말 것.
