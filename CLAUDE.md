...
⚠️ **FAQ·사용설명·홈(히어로 폰 목업) 세 곳이 이 저장소의 번역 파일을 읽어 만들어진다.** `lib/app/core/translations/*.dart`의 **79개 키**를 **이름 변경·삭제하면 페이지 빌드가 실패**한다(`extract_strings.py`가 exit 1). 문구만 바꾸는 것은 안전하며, averic-lab에서 `extract_strings.py`를 재실행하면 20개 언어에 반영된다.
