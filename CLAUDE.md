...
- `GuardianDashboardController._scheduleHeartbeatIfGS()` — G+S일 때만 onInit에서 서버 스케줄 재동기화 + **WorkManager만 재등록** (LocalAlarm 재예약 안 함 — S `_syncScheduleFromServer`와 동일). 안전망 알림 재예약은 `_onHeartbeatSent`가 `forceNextDay`로 단일 소유하므로, onInit에서 `forceNextDay` 없이 재호출하면 이미 오늘 전송돼 내일로 옮겨둔 알림을 (heartbeat+3h가 오늘 미래면) 당일로 되돌리는 버그가 발생한다
...
| 3차 (안전망)   | `lib/app/core/services/local_alarm_service.dart`                                                  | 일일 안부 확인 안전망 로컬 알림 (양 플랫폼 매일 반복, daily repeat via `matchDateTimeComponents.time`). **iOS**: heartbeat 예약 시각 정시에 fire — BGTaskScheduler 미사용으로 사실상 PRIMARY 트리거. **Android**: heartbeat 예약 시각 + 3시간에 fire — WorkManager one-off + periodic 15분 + 앱 열기(2차)가 모두 실패해 worker 자체가 OEM/사용자에 의해 cancel된 케이스를 메우는 LAST-RESORT 안전망. 자정 넘기면 자연 롤오버(예: 22:30 → 다음 날 01:30). 알림 탭은 앱을 **safety_home으로 전환**하고 홈 컨트롤러의 `onInit`/`onResumed`가 미전송 heartbeat 자동 전송. heartbeat 성공 시 `_onHeartbeatSent`가 `schedule(forceNextDay: true)` 호출 → `cancel(_alarmId)`로 예약 + 표시 중 알림을 모두 제거하고 내일자로 재예약 — stale 알림 잔존 없음. **예약시각 변경 시**(`HeartbeatScheduleMixin.onHeartbeatTimeChanged`)에도 `forceNextDay = (이미 오늘 전송됨 || 새 시각이 오늘 지남)`을 조건부 전달 — Android 안전망(heartbeat+3h)이 "방금 지난 시각"으로 바꿀 때 오늘 즉시 발화하던 스퓨리어스 미전송 알림 차단. 이때 미전송+과거 시각이면 `HeartbeatService.execute()`로 **즉시 전송**(오늘분 기록 → 거짓 미수신 경고 방지)하고 성공 시 "안부 전했습니다"(`subject_home_manual_report_sent`) 스낵바, 이미 오늘 전송됨이면 `lastHeartbeatDate` 유지로 재전송 차단. **단일 소유 invariant**: 안전망 알림 재예약은 `_onHeartbeatSent`(전송 성공/실패 시 `forceNextDay`)와 `onHeartbeatTimeChanged`(시각 변경)만 담당하고, **onInit/서버 동기화 경로(S `_syncScheduleFromServer`, G+S `_scheduleHeartbeatIfGS`)는 worker만 재등록하고 LocalAlarm은 재예약하지 않는다** — `forceNextDay` 없는 onInit 재호출이 이미 내일로 옮겨둔 알림을 (heartbeat+3h가 오늘 미래면) 당일로 되돌리던 버그를 차단(과거 G+S onInit에만 누락돼 재설치 후 미래 시각 변경 시 안전망 알림이 당일 발화하던 결함). **로컬 알림 탭 라우팅**: iOS `gs_deadman` / Android `safety_net`·`send_failed` 모두 **safety_home으로 이동**(`fcm_service._routeToSafetyHome` — 역할 인식: G+S는 Dashboard base + push, 순수 S는 단독; kill 런치는 splash가 처리). FCM 푸시(`alert_*` 등)는 알림 목록으로 가는 것과 구분됨. retry 3회 실패 알림(`_sendFailedId`)은 `cancelSendFailed()`로 별도 정리                                                                                                                                                                                                |
