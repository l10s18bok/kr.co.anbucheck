...
│  지정 시각 + 10분 미수신 → 보호자에게 Push 경고 (3일간 반복) │
└──────────────────────┬──────────────────────────────┘
                       │
          ┌────────────┴────────────┐
          ▼                         ▼
...
| 보조 방식             | **로컬 반복 알림 안전망** (Silent Push 수신 시 재예약, 미수신 시 알림 표시하여 앱 실행 유도) — Android/iOS 공통  | **로컬 반복 알림 안전망** (Silent Push 수신 시 재예약, 미수신 시 알림 표시하여 앱 실행 유도) — Android/iOS 공통  |
...
#### 보조 방식: 로컬 반복 알림 안전망 (Android/iOS 공통)

**배경 — BGTaskScheduler(iOS) / WorkManager(Android)를 채택하지 않는 이유:**
...
[heartbeat 전송 성공 시마다 (Silent Push / 앱 포그라운드)]
    ...
    │
    ├─ 2. 매일 반복 로컬 알림 등록 (heartbeat 시각 + 10분, repeats: true)
    │     DateTimeComponents.time 기반 매일 같은 시각 반복
    │     ┌──────────────────────────────────────┐
    │     │ 📱 안부 확인이 필요합니다              │
    │     │                                      │
...
[정상 동작 시]
    ...
    → 데드맨 알림(09:40)이 실제로 표시될 일 없음 (매번 리셋되므로)

[비정상 시 (Silent Push 실패, 앱 강제 종료, 네트워크 끊김 등)]
    당일 09:40 → 1차 알림 표시 (heartbeat 시각으로부터 10분 경과)
    다음 날 09:40 → 2차 알림 표시 (사용자가 1차를 무시한 경우)
    그 다음 날 09:40 → 3차 알림 표시 ...
...
| Android 중국 제조사 배터리 최적화 | 권한 안내 화면에서 Xiaomi 등 중국산 OEM 기기만 자동 감지하여 배터리 최적화 제외 요청 (삼성·Pixel 등 미표시)                                                                                                                                                                                                                                                |
