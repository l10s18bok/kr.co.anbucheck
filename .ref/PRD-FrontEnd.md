...
```
- 대상자가 직접 변경: 서버에 반영 + WorkManager/BGTask 재예약 + 로컬 알림 시각 갱신
...
[정상 동작 시]
    매일 09:30 WorkManager 실행 → heartbeat 성공 → 다음 날 재예약
...
| WorkManager/BGTask (매일 예약 시각)              | 센서 스냅샷(가속도+자이로) 조회 → heartbeat 전송 → 다음 날 재예약 | 없음      |
| 데드맨 스위치 로컬 알림 터치 (대상자)            | `main()` → heartbeat 자동 전송                   | 전체 화면 |
...
| Android 중국 제조사 배터리 최적화 | 권한 안내 화면에서 Xiaomi 등 중국산 OEM 기기만 자동 감지하여 배터리 최적화 제외 요청 (삼성·Pixel 등 미표시) — WorkManager 정상 동작을 위해 필수                                                                                                                                                              |
| 네트워크 장시간 불가 (Android)    | WorkManager 실행 → 네트워크 없음 감지 → 로컬 큐에 적재 + 대상자 폰에 로컬 알림 표시 ("인터넷 연결이 꺼져 있습니다") + 복구 시 일괄 전송 → 서버가 경고 자동 해소                                                                                                                                            |
| 네트워크 장시간 불가 (iOS)        | BGTask 실행 실패 또는 네트워크 없음 → heartbeat 시각 + 10분 후 **데드맨 스위치 로컬 알림** 표시 ("안부 확인이 필요합니다. 앱을 열어주세요") → 사용자가 알림 탭 → 앱 실행 → 네트워크 복구 시 heartbeat 전송 → 서버가 경고 자동 해소. 알림 무시 시 매일 반복 + 서버 경고 플로우로 전환                          |
