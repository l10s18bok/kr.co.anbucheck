...
### 1.2 목적
스마트폰 사용 패턴을 기반으로 사용자의 생존 여부를 자동 감지하는 크로스 플랫폼(Android/iOS) 시스템.
...
- 대상자 모드: heartbeat 전송, 생존확인 서비스 동작
...
- **가속도+자이로 센서 스냅샷 비교**를 보조 생존 지표로 사용
...
```json
// Silent Push - 생존확인용 (사용자 모름, 대상자 기기 대상)
{
...
{
  "aps": {
    "alert": { "title": "안부 확인", "body": "대상자의 생존확인이 정상 복귀되었습니다." },
    "sound": "default"
  },
  "type": "alert_resolved"
}
...
Day 0, 09:30: heartbeat 정상 수신 → 생존 확인됨
...
### 3.6 보호자 알림 이력 로컬 저장 정책

**설계 원칙:** 서버 DB 최소화 — 서버는 현재 활성 경고 상태만 유지하고, 알림 수신 이력은 보호자 기기 로컬에 저장한다.

**역할 분리:**
| 저장소 | 데이터 | 유지 기간 |
|--------|--------|-----------|
| 서버 `alerts` | `active` 상태 경고만 | 경고 해소(cleared) 시 삭제 |
| 클라이언트 `sqflite` | 수신된 Push 알림 이력 | 30일 / 최대 100건 |
...
앱 FCM 핸들러 수신
    ↓
로컬 sqflite에 저장 (title, body, alert_level, received_at)
    ↓
알림 목록 페이지 → 서버 API 호출 없이 로컬 DB 조회
    ├─ 오늘 알림: received_at = 오늘
    └─ 지난 알림: received_at < 오늘
```

**정리 정책:**
- 앱 실행 시마다 30일 초과 항목 자동 삭제
- 100건 초과 시 오래된 순으로 삭제
- 앱 삭제 또는 기기 변경 시 이력 소실 (이 앱의 목적상 허용 가능)
...
└── app/
    ...
    │   │   ├── sensor_service.dart      # 가속도+자이로 센서 조회 (보조 생존 지표)
...
| `sensors_plus`                             | 가속도 센서 조회       | **보조 생존 지표** (폰 위치 변화 감지), 권한 불필요              |
...
| `/api/v1/heartbeat`            | POST   | 생존확인 heartbeat 전송                                |
