...
App Store Connect는 **언어별 "이번 버전의 새로운 기능" 칸에 하나씩 직접 입력**한다(태그 일괄 붙여넣기 불가). 그래서 **언어마다 제목 한 줄 + 본문 코드블록 하나**로 출력해 한 칸씩 복사하기 쉽게 만든다. **각 언어 4000자 이내.**

⚠️ **제목(`■ 언어 / 코드`)은 코드블록 밖에 둔다.** 코드블록 복사 버튼을 누르면 본문만 복사돼야 한다 — 제목이 블록 안에 있으면 App Store Connect 입력 칸에 `■ 중국어 번체 / zh-Hant`까지 같이 붙는다(2026-09-17 사용자 지적). 형식은 반드시 아래와 같다:
...
⚠️ **출력 순서는 아래 목록 그대로다**(사용자 지정 순서, 2026-09-17 — 임의로 바꾸지 말 것). 로케일 코드도 Play와 다르므로 아래 표기를 쓴다:
