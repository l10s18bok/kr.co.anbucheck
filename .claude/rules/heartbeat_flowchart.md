...
flowchart TD
    ...
    StepsCheck -->|NO 또는 null| Suspicious[suspicious = true<br/>활동 의심]
...
>
> **iOS 로컬 알림 예약 정책 (isReportedToday 기반 1회성/매일반복 분기)**: `flutter_local_notifications` `zonedSchedule`에 `matchDateTimeComponents: DateTimeComponents.time`을 쓰면 iOS는 날짜를 무시하고 다음 HH:mm 발생 시각을 찾아 발화한다 — heartbeat를 예약시각 전에 전송해도 예약시각에 알림이 또 트리거되는 버그가 발생한다. 이를 방지하기 위해 `LocalAlarmService.schedule()`은 `forceNextDay`(= `isReportedToday`) 와 오늘 예약시각 경과 여부(`todayNotYetFired`)를 조합해 분기한다: `forceNextDay=true` + `todayNotYetFired=true`(오늘 이미 전송 완료인데 예약시각이 아직 안 지남) → `matchDateTimeComponents=null`로 내일 날짜+시각을 명시한 1회성 알림. 그 외 → `DateTimeComponents.time`으로 매일 반복 알림. `forceNextDay` 결정 기준: `lastHeartbeatDate == 오늘 날짜` (`isReportedToday`). 이 로직은 heartbeat 성공 후(`_sendOrSavePending`), 앱 진입 시 스케줄 동기화(`_scheduleHeartbeatIfGS`), 예약시각 변경(`onHeartbeatTimeChanged`) 세 곳 모두에서 적용된다.
