...
| 용어 | 값 | 의미 |
|------|-----|------|
| `suspicious` | `true` | **활동 증거가 하나도 없음.** 구제 조건이 전부 실패했을 때만 참이 된다 |
| `suspicious` | `false` | 구제 조건 중 **하나라도** 참 — 활동 기록 확인 |

**구제 조건은 플랫폼마다 다르다** (묻는 질문은 같다 — *"전송 시점에 사람의 조작 흔적이 있는가"*).

| | Android | iOS (확장, 2026-09-01 도입) |
|---|---|---|
| 활동 | `steps_delta > 0` | `steps_delta > 0` |
| 조작 | `isInteractiveAtTrigger`(worker fire 시점 **화면 켜짐**) | **잠금 해제 상태**(확장이 키체인 프로브로 판별, 6/6 검증) |
| 추가 | — | **오늘 앱을 포그라운드로 열었는가**(App Group 마커, 하루 단위) |

⚠️ **iOS 쪽 신호가 더 정확하다** — 안드로이드의 "화면 켜짐"은 **알림 하나로도 참**이 되어
사람이 아무것도 안 해도 구제되지만, 잠금 해제는 Face ID나 암호를 요구한다.

⚠️ **iOS는 2026-08-22~08-31 동안 `false` 고정이었다.** 근거는 *"확장에 화면 상태 신호가
없다"*였고 그 전제가 08-31에 깨졌다. **`false` 고정으로 되돌리지 말 것** — 상세와 되돌리는
법은 PRD-FrontEnd §2.2.1 규칙 5 / `ios_nse_field_notes.md` §18~19.

⚠️ **서버는 플랫폼을 구분하지 않는다.** 받은 `suspicious`를 양 플랫폼 **같은 경로**로
처리한다(1회 주의 → 2회 경고 → 3회+ 긴급). 아래 차트 2·3의 판정 흐름이 iOS에도 그대로 적용된다.
...
flowchart TD
    ...
    Classify -->|"recovery_날짜 (살아있음 신호)"| Liveness[이력 적재 + 생존 확인<br/>battery_level만 갱신 — last_seen·steps_delta 미갱신<br/>활성 경고 해소 + suspicious_count 리셋<br/>→ 보호자에게 정상 복귀 Push, 대시보드 카드 정상 복귀<br/>auto_report 미발송 — 정시 전송이 몇 시간 뒤 같은 알림을 보냄<br/>당일 첫 수신으로 세지 않음]
...
**기기 시간대 동기화 (2026-09-25):** 위 차트의 "도착일"·"오늘 로컬 자정"은 모두 `devices.timezone` 기준이다. 서버는 heartbeat 처리 **맨 앞(분류 전)**에서 payload의 `timezone`이 저장값과 다르고 유효하면(`ZoneInfo`·`pg_timezone_names` 양쪽 통과) 먼저 갱신한 뒤 새 값으로 분류한다 — 순서가 바뀌면 해외로 이동한 첫 전송이 옛 시간대로 지난 기록 보정에 떨어진다. 키 종류와 무관하게 갱신하며, 실패해도 heartbeat는 막지 않는다. 클라는 시간대를 **전송 직전**에 싣고 보류 큐에는 저장하지 않는다. `heartbeat_hour/minute`는 "현재 있는 곳의 현지 시각"이다. 상세: PRD-FrontEnd §2.2.3 / PRD-BackEnd §4.6.
...
> - **iOS (2026-08-22 변경 — 서버 푸시 + 확장이 PRIMARY)**: 예약시각 **정각**에 서버가 `heartbeat_push`를 보내고, **Notification Service Extension이 앱이 강제 종료된 상태에서도 실행돼 heartbeat를 직접 전송한다** — 사용자가 탭할 필요가 없다. 성공하면 확장이 문구를 "전달 완료"로 바꾸고 무음·배너 없음(`.passive`)으로 내린다. 실패하거나 확장이 실행되지 못하면 원본 문구(탭 유도)가 그대로 표시돼 **기존 동작으로 안전하게 되돌아간다.** 정시 로컬 알림 `gs_deadman`은 더 이상 예약하지 않으며(잔존 기기의 탭 라우팅만 유지), 로컬 알림은 **오프라인 전용 폴백**(`anbu_offline_<날짜>`, 예약시각 **+45분**, 7일 롤링)으로 남는다 — 망이 있으면 확장이 그날치를 지우고, 망이 없으면 푸시가 안 와 그대로 발화한다. 실측 근거는 `.claude/rules/ios_nse_field_notes.md`. **피기백(2026-08-29)**: APNs 보관 슬롯이 앱당 1칸이라 보호자 알림 하나가 트리거를 밀어내 영구 소실시킨다. 그래서 확장이 보호자 리포트 계열 푸시(`auto_report`·`manual_report`·`steps`·`battery_*`·`alert_resolved`·`alert_cleared`)에도 **오늘 미전송이면 안부를 얹어 보낸다** — 알림 본문은 원본 그대로, 리포트 계열은 서버 변경 없음(늦은 경고 판정만 서버 `sent_at` 사용). 전송 성공 시 지난 7일치의 **표시된** 폴백도 지운다(오늘치 pending은 유지). 긴급·경고 계열은 지연 금지라 제외 — 단 `alert_caution/warning/urgent`가 `sent_at` 대비 30분 이상 **늦게 배달**된 경우만 얹는다(2026-09-16, `alert_emergency`는 항상 제외). **대상자 가드**(`hb_is_subject`)가 없으면 순수 보호자 기기가 자기 heartbeat를 보내게 되므로 필수다. 알림 자체에서 heartbeat를 전송하지 않고, 사용자가 탭하면 앱이 열려 `isReportedToday=false`일 때만 전송한다 — **G+S**: `fcm_service._handleNotificationTap`가 `refreshAndForceSend()`를 직접 호출(미전송 시만 전송, 이미 전송됐으면 다이얼로그만, Android `subject_safety_net` 탭과 동일 패턴). iOS는 보호자 전용이라 순수 S 모드가 없다. **iOS 알람 등록 불변 규칙**: `matchDateTimeComponents.time`으로 OS가 매일 자동 반복하므로 heartbeat 성공 후 재등록·취소가 불필요하다. `_onHeartbeatSent`는 iOS 알람을 건드리지 않는다. 등록 시점은 (1) 최초 설치(`enableSubjectFeature`/온보딩 완료 시), (2) 재설치(`_scheduleHeartbeatIfGS` onInit — 동일 ID 덮어쓰기, idempotent), (3) 예약시각 변경(`onHeartbeatTimeChanged`) 3곳만이다.
