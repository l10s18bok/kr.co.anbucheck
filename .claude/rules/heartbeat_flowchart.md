...
flowchart TD
    ...
    Trigger{트리거 종류?}
    Trigger -->|고정 시각 Android| WM[WorkManager 2계층<br/>one-off 정확 발화 + periodic 15분 폴링<br/>예약시각 -15분 이전 fire는 평소 스킵<br/>단 미전송 갭 + 화면 활성이면 회복 전송<br/>전송 성공 시 _onHeartbeatSent가<br/>schedule 호출 → one-off + periodic 둘 다<br/>cancel + 내일자 register<br/>race 방어: lastScheduledKey 성공 마커<br/>+ HeartbeatLockDatasource SQLite UNIQUE CAS<br/>cross-isolate 원자 락, TTL 30초]
    ...
    WMRecov -->|YES + 미전송 갭<br/>+ 화면 활성| Recovery
    WMRecov -->|YES 그 외| End0
    BG --> Collect
    FG --> FGCheck{예약 시각 지남<br/>AND 오늘 미전송?}
    FGCheck -->|YES| Collect
    FGCheck -->|NO 미전송 갭| Recovery
    FGCheck -->|NO 그 외| End0([종료 — 이미 전송 완료 / 정시 대기])

    Recovery[회복 전송 _executeRecovery<br/>예약시각 이전 살아있음 신호<br/>steps_delta=null, suspicious=false<br/>전용 키 recovery_날짜 + 마커 lastRecoveryDate<br/>정시 슬롯 미소비 — lastHeartbeatDate/<br/>lastScheduledKey 미갱신, 재예약 안 함<br/>실패 시 pending 큐 미사용<br/>Android 전용]
    Recovery --> RecovEnd([종료 — 예약시각 정시 전송은 그대로 수행])
