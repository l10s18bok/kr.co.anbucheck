...
**해법: `devices.app_version` 컬럼 추가 + NULL을 구버전으로 간주.**
구버전 클라는 앱 버전을 영영 올려보내지 않으므로 NULL로 남는다 — 즉 **NULL = 구버전**이
저절로 성립한다. 새 iOS 푸시는 `app_version IS NOT NULL`인 기기에만 보내면
구버전 사용자는 **현행 동작 그대로**(로컬 알림 하나)를 유지한다.
