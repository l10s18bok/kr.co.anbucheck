...
### ⚠️ 직접 원인은 33초가 아니라 **11초짜리 엔진 부팅**이다 (정정)

처음에 "걸음수 조회 33초 때문"이라고 적었으나 틀렸다. Dart에는 **3초 타임아웃**이 있다
(`_getStepsDelta`의 `.timeout(const Duration(seconds: 3))`). Dart는 33초를 기다리지
않고 3초에 포기해 `steps=null`로 넘어갔고, 33초는 네이티브 콜백이 뒤늦게 도착해 로그에만
남은 것이다.
...
n=1로는 구분되지 않는다. 다만 결과는 분명하다 — **그 조건에서 3초 타임아웃은 부족하고
`steps_delta=null`이 된다.** 그러면 화면이 꺼져 있어(`isInteractive=false`)
`suspicious=true`가 되어 **보호자에게 매일 "활동 기록이 감지되지 않습니다" 거짓 주의
알림**이 나가고 걸음수 차트도 0이 된다. 전송이 되기 시작하면 곧바로 드러날 문제다.
