...
**경위 — 첫 시도(09-13, +30m 데이터 전용)는 실패였다(§8.10.6).** FCM 문서가 *"messages don't
...
- ~~데이터 전용 FCM도 temp-power-save를 받는가~~ → **§8.10.6에서 사실상 부정.** 표시형으로 교체
...
#### 8.10.6 ★ 데이터 전용 푸시는 차단을 뚫지 못했다 — A66-Q1OR (09-14, 실사용자)
...
| 09-12 | 19:31 | |
| 09-13 | 18:05 | 정상 |
| **09-14** | 18:30 시점 **미전송** | 09-11 +3일 → 재편입으로 보임 |

```
09-14 18:30:05  [사일런트 깨우기] 발송 — 대상 3건 / 성공 3건 / 261ms
                ← A66-Q1OR 조건 전부 일치(android·18:00·Asia/Seoul·토큰 있음·last_seen 09-13)
18:30 ~ 18:59   이 기기 안부 없음
```

**판정: 데이터 전용 고우선순위 FCM(FCM 수락)은 이 기기의 차단을 열지 못했다.** 같은 기기가 +2h
표시형 알림에는 예외 없이 반응해 왔다(사용자 확인: 한 번도 실패한 적 없음). FCM 문서의 강등 규칙과
정합한다. ⚠️ "18:30에만 오프라인이었다" 가능성은 남지만 5일 연속 정확히 반응한 기기라 낮다.

→ 같은 날 서버를 **+60m 표시형 조기 미전송 알림**으로 교체했다(§8.5 대응).

⚠️ 부수 확인: 사일런트 성공 로그에는 기기 식별자가 없어 **"대상 3건이 누구인지"를 로그로 알 수 없었다.**
표시형은 `send_push`가 `subject_safety_net (토큰 앞 10자)`를 남기므로 이 구멍이 함께 메워진다.
