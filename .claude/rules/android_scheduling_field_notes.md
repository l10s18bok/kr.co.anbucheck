...
쿼터 회복까지 기다렸다가 서버 미수신 체크(09:00)에 걸려, 기기가 멀쩡한데도 미수신 경고가 나갔을 것이다.
...
WorkManager 단독 실측(+1h27m ~ +2h52m, 상한 없음) 대비 **10~20배 개선**이다.
상한은 **문서상 60분**이고 이 기기 실측도 그 안에 들어왔지만 **보장은 아니다** — 상세는 §6.
...
**→ 2026-09-16, 이 건을 근거로 문구를 고쳤다.** `notifySendFailed`가 iOS 오프라인 폴백과
**같은 문구**(`offline_alarm_title`/`offline_alarm_body`)를 쓴다 — 번역은 이미 20개 언어에
있고 캐시 목록에도 올라 있어 **코드 한 곳(키 두 줄)만 바뀌었다.**

```
제목  📶 인터넷 연결을 확인해주세요   →  💗 안부를 아직 전하지 못했습니다
본문  이 메시지를 터치하면 자동으로 재전송합니다
      →  이 알림을 한 번 눌러 주세요. / 눌러 주시면 보호자에게 안부를 전합니다
```

이 알림이 참인 조건은 하나뿐이다 — **오늘 안부가 아직 나가지 않았다.** 본문도 안드로이드에서
참이다(탭 → 앱 진입 → 2차 안전망 전송). ⚠️ **원인을 단정하는 제목으로 되돌리지 말 것.**
옛 키(`notification_send_failed_title`/`_body`)는 번역 파일에 남지만 참조되지 않는다.

⚠️ **문구만 바뀌었고 전송이 막히는 원인은 그대로다.** 이 기기에서 방화벽을 연 것은 우리 앱으로
들어온 FCM이었고(§7.8), 앱이 스스로 열 수단은 없다. 남은 후보는 "지속 실패에서만 띄우기"인데
실측 근거가 없어 보류한다.
...
```
삼성은 08-26·08-27 배달이 `elapsed` 기준 **정확히 86,400,000ms 간격**이었다.

> ⚠️ **여기에 "시스템이 같은 시각에 도는 다른 알람과 묶어서 배달한다"는 해석을 달아 뒀으나
> 2026-08-31에 반증됐다(§7.7).** 86,400,000ms는 24시간이므로 "같은 시각 반복"의 산술적
> 결과일 뿐 배치의 증거가 아니었다. 예약시각을 07:00 → 20:30으로 옮겼는데도 오프셋(+29분대)이
> 그대로 재현됐다. 딸려 있던 결론 — *"요청 시각을 앞당겨도 배달 시각은 따라오지 않으므로
> 알람을 몇 분 먼저 쏘자는 방향은 성립하지 않는다"* — 도 근거가 무너졌다. **미확인으로 되돌린다.**
> (`setExactAndAllowWhileIdle`을 쓸 수 없다는 것은 별개 제약이며 그대로다.)
...
⚠️ **이 절의 값은 전부 "배터리 최적화가 적용된" 조건이다. 조건을 명시하지 않은 발화 시각과
비교하지 말 것** — 최적화에서 제외된 기기는 완전히 다른 수치가 나온다(§7.2, 9일 연속 +1~7분).
...
→ 2026-08-31 샤오미에서 **"발화 시점이 유지보수 창이었다"는 것까지는 실측으로 보탰다**
(§7.8.3). 그러나 (가)/(나)를 가르지는 못했다 — 위 두 갈래는 그대로 유효하다.

### 핵심 차이는 평균이 아니라 **상한**이다

```
WorkManager :  +1h27m ~ +2h52m,  상한 없음 (창이 6시간까지 배증)
알람        :  +7분 ~ +32분,     상한 60분 (문서상 — 보장 아님, 아래 참조)
```

⚠️ **"1시간 이내"는 몇 번의 실측에서 거의 지켜졌을 뿐 확실한 것이 아니다.** 깨지는 경로가
넷이고 전부 이 문서 안에 실측으로 남아 있다:

1. **OEM 정책이 창 자체를 무효화한다.** MIUI `power_pending`이 요청 시각에 +72시간을 더해
   `whenElapsed`·`maxWhenElapsed`가 **둘 다** 3일 뒤가 됐다(관측 3회). 그날은 1시간 창이
   존재하지 않았다.
2. **알람이 떠도 전송이 안 될 수 있다.** 08-28 MIUI는 예약 +35초에 발화하고도 `APP_STANDBY`
   방화벽에 막혀 실패했고(그날 안부는 FCM으로 +59분), 08-23 삼성은 창이 11.1초에 닫혀
   전송이 창 밖으로 밀렸다.
3. **알람이 무장조차 안 될 수 있다.** MIUI가 `MY_PACKAGE_REPLACED`를 차단해(n=4) 업데이트
   후 알람 0건이 됐고, 08-29 샤오미는 발화하지 못한 날 재무장이 통째로 밀렸다.
4. **API 31 미만이면 알람이 떠도 아무 일도 안 한다** — `enqueueWindowHolder`/`enqueueHeartbeat`가
   `SDK_INT < S`에서 즉시 return한다. minSdk 29이므로 Android 10·11 기기가 해당한다.

표본도 좁다 — 알람 발화 실측은 **SM-A325N 1대 + Redmi 1대**가 전부다.

제품에서 중요한 건 정시성이 아니라 **서버 미수신 체크(예약시각 +2h)보다 먼저 도착하는가**다.
알람은 최악 +60분이어도 1시간 여유가 남지만, WorkManager는 실측에서 이미 그 선을 넘어
**기기가 멀쩡한데도 미수신 경고 구간에 들어갔다**(08-19·08-20).

⚠️ **이걸 "거짓 경고"라 부르지 말 것.** 미수신 경고가 주장하는 명제는 *"오늘 안부
확인이 없습니다"* 하나이고, 실제로 안 왔으면 **참**이다. 거짓인 것은 경고가 아니라
그 경고에서 사람이 읽어내는 함의(*"대상자에게 문제가 있다"*)이며, 경고는 그런 주장을
하지 않는다(FAQ도 "무사하신데 전달만 늦어진 경우에도 이 알림이 갈 수 있습니다"라고
안내한다). 0차 계층의 근거는 "거짓 경고를 막는다"가 아니라 **"정상 기기가 미수신으로
판정되는 구간에 들어가지 않게 한다"**이다.
...
## 7. 2026-08-30 관측 — 알람이 워커 실패를 구했다 / 조건을 명시하지 않은 비교의 위험

### 7.1 ★★ 알람이 워커 2회 실패를 구한 첫 종단 사례 (SM-A325N, 예약 20:30)

지금까지 알람 계층의 성공 사례(08-24)는 **워커가 실패하지 않은 상황**이었고, 08-25·08-28은
알람이 실패하고 FCM이 구한 사례였다. 이번은 **"워커 실패 → 알람 구제"가 한 사이클 안에서**
관측된 첫 건이다.

```
20:32:45  HeartbeatSend: FAIL src=one-off  attempt=3 unreachable=true   (예약 +2분 45초)
20:32:46  send_failed 알림 게시 (ID 1397121385)
20:33:04  HeartbeatSend: FAIL src=periodic attempt=3 unreachable=true
20:34:08  ★ FIRED idle=false bucket=10                                  (예약 +4분 8초)
20:34:08  HOLD enqueued / HB enqueued → 워커 2개 시작
20:34:08  DNS 요청 3건 → 성공
20:34:09  HOLD ended held=1506ms reason=heartbeat-done
```

**결정적 증거는 DNS 요청 분포다.** `NetdEventListenerService`의 uid 11227 요청을 초 단위로 세면:
...
그사이 바뀐 것은 알람이 연 **expedited 창**뿐이다. 망은 20:29부터 붙어 있었고
(`NetworkState(isOnline=true, isWifiConnected=true)`) 기기는 딥 Doze였다 —
**망 문제가 아니라 방화벽이었고, 창이 그것을 열었다.**

⚠️ **성공 전송에는 `HeartbeatSend` 로그가 없었다.** 보류 큐 재전송(`_sendPendingInternal`)이
계측 함수를 거치지 않던 구멍이며 **1.3.2+57에서 로그를 추가해 메웠다**(`pending=true` 필드로
새 전송과 구분된다). 그전까지는 이 사례처럼 DNS 타임스탬프를 뒤져야 했다.

⚠️ **발화가 +4분 8초로 빨랐던 것은 성능이 아니라 조건이다** — `bucket=10`(ACTIVE)이었다.
필드노트의 +29분은 전부 RARE(40) 관측이므로 **나란히 적지 말 것.** 무장 시점 덤프에서도
`app_standby`/`battery_saver`/`tare`가 전부 음수(미적용)였고 `whenElapsed`가 정각이었다.

### 7.2 ⚠️ 배터리 최적화 **제외** 상태의 WorkManager 단독 기준선 (Galaxy S10/Note10, Android 12)

실사용자 기기(adb 불가, 서버 `heartbeat_logs`로만 관측). 예약 18:00, `queue_min` 전 구간 0.

| 날짜 | fire_delay | | 날짜 | fire_delay |
|---|---|---|---|---|
| 08-20 | +1분 | | 08-25 | +3분 |
| 08-21 | +7분 | | 08-26 | +1분 |
| 08-22 | +7분 | | 08-27 | +1분 |
| 08-23 | +1분 | | 08-28 | +2분 |
| 08-24 | +3분 | | 08-29 | **+102분** (업데이트일) |

**9일 연속 +1~7분이 0차 계층 없이(구버전) WorkManager 단독으로 나왔다.** 같은 제조사인
SM-A325N의 +1h27m~+2h52m과 **비교 대상이 아니다** — 이 기기는 그 기간 **배터리 최적화에서
제외**돼 있었다(사용자가 08-29에 최적화를 켰다). Doze 제약을 통과했으므로 "Doze 건 아니건
제때 발화했다"는 관찰과도 일치한다.

**필드노트에 없던 조건의 기준선이라 버리지 않고 남긴다.** 동시에 판독 규칙이 하나 늘었다:

> **최적화 제외 여부를 명시하지 않은 발화 시각은 다른 기기와 비교할 수 없다.**
> `mState`(§4)와 standby 버킷만으로는 부족하다.

08-29의 +102분은 **업데이트와 최적화 전환이 같은 날 일어나** 변수가 둘이므로 판독 불가다(폐기).

### 7.3 MIUI `power_pending` — 앱 사용은 해제 조건이 아니다 (반증 2건)

08-26에 `power_pending`이 `--`로 풀린 것을 보고 *"앱 사용이 해제 조건이라면"*이라는 추정을
적어 두었다. 같은 날 두 번 반증됐다(Redmi 23021RAA2Y, **표본 1대**).

```
08-30 18:33  그날 앱을 9회 열고 17:57에 예약시각을 바꿔 새로 무장했는데도
             power_pending=+3d2h26m46s   →  whenElapsed 동일 (배달 ≈ 09-02 21:00)
08-30 21:35  1.3.2+57 업데이트 + 앱 실행 직후에도
             power_pending=+3d23h24m55s  →  배달 ≈ 09-03 21:00
```
...
같은 시각(21:35), 두 기기 모두 **앱을 연 지 3~4분** 시점.

| | 버킷 | `mState` |
|---|---|---|
| SM-A325N (삼성) | **ACTIVE (10)** | `INACTIVE` |
| 23021RAA2Y (MIUI) | **RARE (40)** | **`IDLE`** (딥 Doze) |

AOSP 기준 딥 Doze 진입은 화면 끄고 최소 35분(`inactive_to 15분 + idle_after_inactive_to 20분`)인데
MIUI는 4분 만에 들어갔고, 방금 앱을 썼는데도 버킷이 RARE다.

⚠️ **§5의 "앱을 열면 standby 버킷이 ACTIVE로 리셋된다 → 테스트 조건이 유리해진다"는 경고는
삼성 기준이며 MIUI에는 적용되지 않을 수 있다.** 그 전제로 샤오미 관측을 해석했다면 다시 볼 것.

### 7.5 `MY_PACKAGE_REPLACED` — 삼성 통과 / MIUI 차단 (n=4)

1.3.2+57 업데이트 직후.

```
[삼성]  21:32:50.317  ARMED (app-process-start) for=08-31 20:30
        21:32:50.413  ARMED (system:android.intent.action.MY_PACKAGE_REPLACED) for=08-31 20:30
[MIUI]  해당 로그 없음 — app-process-start는 전부 사용자가 앱을 연 결과
```

MIUI는 이 브로드캐스트로 프로세스를 띄우지 않으므로 **앱을 열지 않았다면 알람·job 0건으로
잠들어 있었다.** 복구 경로는 여전히 셋뿐이다 — FCM 도착 / 재부팅 / 앱 열기.

⚠️ 다만 **사용자에게 "업데이트 후 앱을 열어 주세요"를 요구하지 않는다**(이 앱의 주 대상은
앱을 열지 않는 사용자다). 순수 S 사용자에게는 서버 미수신 체크의 `subject_safety_net`
(예약시각 +2h)이 도착하며 그 FCM이 프로세스를 깨워 자가 복구시킨다(08-24 12:06 실측).
**대가는 그날 하루 미전송 + 보호자에게 경고 1건이고, 다음 날부터 정상이다.**

### 7.6 부수 — 예약시각 변경이 "첫 설치"를 사칭하던 결함

이날 진단 중 heartbeat 로직 결함을 하나 찾아 고쳤다(1.3.2+57). `onHeartbeatTimeChanged`의
미전송 분기가 `lastHeartbeatDate`를 비워, 다음 앱 진입에서 `_checkAndSendHeartbeat`가
"첫 설치"로 오인하고 **새 예약시각 전에 전송**하던 문제다. 스케줄링 계층과는 무관하며
상세는 **PRD-FrontEnd §2.2 "시각 변경 시 예약 트리거 정책"**에 있다.

⚠️ 진단 관점에서만 기억할 것: **그 결함이 살아 있던 빌드에서는 `fire_delay_min`이
알람·워커와 무관한 포그라운드 시각으로 찍힐 수 있다.** 1.3.2+57 이전 데이터를 읽을 때
예약시각 변경이 있었는지 확인하지 않으면 발화 시각 분석이 오염된다.

### 7.7 ⚠️ +29분대 오프셋이 예약시각을 바꿔도 재현됐다 — "배치 파트너" 가설 반증 (2026-08-31)

SM-A325N의 예약을 **07:00 → 20:30으로 13시간 반 옮겼는데 오프셋이 그대로 나왔다.**

| 날짜 | 예약 | 발화 | 오프셋 | 버킷 | `idle` |
|---|---|---|---|---|---|
| 08-25 | 07:00 | 07:29:17 | +29m17s | RARE(40) | true |
| 08-26 | 07:00 | 07:29:16 | +29m16s | RARE | true |
| 08-27 | 07:00 | 07:29:16 | +29m16s | RARE | true |
| 08-28 | 07:00 | 07:29:17 | +29m17s | RARE | true |
| 08-29 | 07:00 | 07:29:16 | +29m16s | RARE | true |
| 08-30 | 20:30 | 20:34:08 | **+4m8s** | ACTIVE(10) | **false** |
| **08-31** | **20:30** | **20:59:17** | **+29m17s** | **FREQUENT(30)** | true |
| 09-13 | 19:30 | 19:59:17 | +29m17s | RARE(40) | true | ← appop 해제 다음 날(§8.10.4)

**반증된 것 둘**

1. **"같은 시각에 도는 다른 알람과 묶여서 배달된다"** — 시각을 옮겼는데 오프셋이 따라왔다.
   재현되는 것은 *시각*이 아니라 **오프셋 자체**다.
2. **버킷이 원인이라는 설명** — RARE(40) 5회와 FREQUENT(30) 1회가 같은 값이다.

**남은 유력 변수는 딥 Doze지만 확정할 수 없다.** 반대 사례가 08-30 하루뿐이고, 그날은
`idle=false`와 `bucket=10`이 **함께** 바뀌었다(변수 2개). 원인은 **미확인**으로 둔다.

⚠️⚠️ **표본은 SM-A325N 1대다. "삼성은 +29분"으로 일반화하지 말 것.**
같은 날 다른 삼성(Galaxy S10, 실사용자 폰, 최적화 적용 상태)은 예약 18:00 → **18:11 전송
(+11분)**이었다. 다만 그 폰은 **셀룰러 실사용 폰이라 그 시각에 사용자가 쓰고 있었을 수 있고
adb도 못 붙여 `mState`를 모른다** — 조건이 다른 값이지 반례로 쓸 수 있는 값이 아니다.

#### 그날의 전송 체인 (4줄 판독 그대로)

```
20:59:17.019  ARM skipped (app-process-start) — 오늘 발화 창 유지 (20:30)   ← 가드 정상
20:59:17.027  ARMED (refire) for=09-01 20:30:00                             ← force 재무장
20:59:17.041  FIRED idle=true bucket=30
20:59:17.046  HOLD enqueued / HB enqueued
20:59:17.449  HOLD started budget=90000ms
20:59:28.225  ContentValues: result: 0                                      ← 걸음수 조회 11초
20:59:30.028  HeartbeatSend: OK src=periodic attempt=1 key=2026-08-31_20:30 steps=0
20:59:30.505  HOLD ended held=13058ms reason=heartbeat-done
```

- `attempt=1`로 한 번에 성공 = **방화벽이 열려 있었다.**
- 걸음수 조회가 **11초** 걸렸다(08-30은 0.2초). 90초 예산이 여유 있게 덮었다 — 08-23의
  33초 사례와 함께, 이 조회 시간이 예측 불가하다는 근거가 하나 늘었다.
...
**후속 확인 — 발화 시점은 유지보수 창이었다.** 우리 로그의 `idle=false`와 시스템의
`nowIdling=true`가 어긋나 보이지만 모순이 아니다. `PowerManager.isDeviceIdleMode()`는
`IDLE_MAINTENANCE` 동안 **false**를 반환한다.

```
21:07:14.539  nowIdling=true,  mode=0        ← 딥 Doze 진입
21:07:18.295  FIRED  idle=false              ← 4초 뒤 발화 (우리 로그)
21:07:19      전송 성공
21:08:54      mState=IDLE_MAINTENANCE        ← 직접 관측
21:09:48.909  nowIdling=true,  mode=2        ← 유지보수 종료 후 딥 Doze 복귀로 보임
21:31:02      mState=IDLE
```

**유지보수 창이 대략 21:07~21:09에 열려 있었고 발화·전송이 그 안이었다.**

⚠️ **그래도 §6의 두 갈래는 못 가른다.**
- MIUI는 유지보수 **진입**을 `nowIdling=false`로 로깅하지 않는다(전이 로그가 없다).
  21:09:48의 `mode=2`가 복귀로 보이지만 `mode` 값의 의미는 MIUI 고유라 단정할 수 없다.
- **인과 방향이 로그로 구분되지 않는다** — (가) 알람 배달이 창을 열었다 / (나) 창이 열려서
  알람이 배달됐다. 딥 Doze 진입 **4초 뒤**에 첫 유지보수 창이 열리는 것은 이상하므로
  (보통 1시간) (가)가 더 그럴듯하지만 **증거로 가른 것은 아니다.**

**§6의 "미확정" 판정은 유지한다.** 이번 관측이 보탠 것은 *"발화 시점의 기기 상태가
`IDLE_MAINTENANCE`였다"* 하나이며, 다음에 같은 것을 볼 때 처음부터 다시 재지 않도록 적어 둔다.
...
#### 대응 — 같은 미전송 알림을 1시간 먼저 보낸다 (2026-09-14, 서버 `job_early_safety_net`)

위 판정의 실질적 피해는 "안부가 안 나간다"가 아니라 **"미수신 판정이 안부보다 먼저 난다"**였다
(§8.10.2). 그래서 서버가 **예약시각 +60m까지 미전송인 Android 기기에 +2h와 동일한
`subject_safety_net` 표시형 알림**을 먼저 보낸다(보호자 경고 없음).

```
예약시각   0차·1차 — 앱옵에 막힘
  +60m  ★ 조기 미전송 알림 → 문 열림 → 밀려 있던 알람·job이 전송 → 같은 태그 알림 자동 삭제
  +2h     미수신 체크(기존, 무변경) — +60m에 뚫렸으면 last_seen이 오늘이라 자동 제외
```

**경위 — 첫 시도(09-13, +30m 데이터 전용)는 문서 근거로 교체했다.** 실패를 직접 보인 실측은 없다(§8.10.6). FCM 문서가 *"messages don't
result in user-facing notifications … may be deprioritized to normal priority"*라고 명시하며,
강등된 메시지는 Doze 중 배달이 지연된다. **문을 여는 것은 사용자에게 보이는 알림이다.**

- **+60m인 이유**: 알람 배달 창 [예약, +60분](`window=+1h0m0s`). +30m이면 창 후반에 뜨는 정상 기기가
  매일 알림음을 듣는다(09-13 테스트폰은 +30m 20초 전 전송).
- **소리·진동이 난다** — 앱 채널이 `anbu_alerts`(high) 하나뿐이고 안드로이드 8+는 채널이 소리를 정한다.
- `AND d.platform = 'android'` 필수(iOS APNs 슬롯 보호, `ios_nse_field_notes.md` §13.5).
- **판정 방법**: 차단 기기의 `HeartbeatSend: OK`가 +2h가 아니라 **+60분대**로 앞당겨지는가,
  `netpolicy`에 그 시각 `reason=PUSH_MESSAGING` 항목이 찍히는가.
...
- `areAlarmsRestricted` 경로가 09-04 알람 전환의 직접 원인인지 — **§8.10.4에서 강한 정황으로 격상**(같은 기기 A/B: `ignore` → 21:30 고정 / `allow` → +29m17s). 단 변수 셋이라 "확정"은 아니다
- **데이터 전용 FCM도 temp-power-save를 받는가** — **여전히 미답**(§8.10.6, 판정 불가). FCM 문서의 강등 규칙을 근거로 표시형으로 교체했으므로 더 재지 않는다
- **+60m 표시형 조기 알림이 차단 기기를 +60분대에 뚫는가** — 다음 차단 기기 관측에서 확인
...
#### 8.10.3 09-13 기준선 실측 (12:31, 관측 7시간 전) + 그날 배포한 +30m 잡

**세 변수가 §8.10.1의 예측과 일부 달랐다 — 결과를 읽을 때 이 값을 쓸 것.**

| 변수 | 차단 기간(09-04~09-12) | 09-13 12:31 실측 | 비고 |
|---|---|---|---|
| `RUN_ANY_IN_BACKGROUND` | `ignore` | **`No operations. / Default mode: allow`** | 해제 확인 |
| `readyNotRestrictedInBg` | `false` ×2 | **`true` ×2** | 게이트 풀림. 지금 막는 건 `TIMING_DELAY`뿐 |
| standby 버킷 | 20 (`reason=f` 강제) | **40 (RARE, `reason=p` 예측)** | ⚠️ 예측은 "20~30"이었으나 **더 내려갔다.** 오히려 §7.7의 +29분 5일치와 **같은 버킷**이라 비교에 유리하다 |
| 알람 무장 | 백그라운드 프로세스에서 | 포그라운드에서(어제 21:41 앱 실행) | §8.10.1의 세 번째 변수 그대로 |

```
mScreenOn=false  mCharging=false  mState=IDLE  (딥 Doze)
origWhen=2026-09-13 19:30:00.000  window=+1h0m0s0ms  power_pending 없음(삼성)
whenElapsed=+6h58m26s  maxWhenElapsed=+7h58m26s     → 배달 창 [19:30, 20:30]
JOB #626 earliest=+7h1m (19:32 periodic) / #627 earliest=+6h58m (19:29 one-off)
RARE: sessionCountInWindow=1 / limit 3               → 쿼터 여유
usagestats: used=+14h44m42s = 어제 21:46 마지막 포그라운드 사용
```

**재편입 예상일**: 마지막 사용 09-12 21:46 + 약 3일(§8.4) = **09-15~16**. 그때까지 앱을 열지 않아야
차단 상태를 다시 관측할 수 있다.

⚠️ **같은 날 12:35에 서버 `job_silent_wake`(예약시각 +30m = 이 기기 20:00)를 배포했다.**
관측이 오염되지 않는 이유와 판별법:

- **알람이 19:30~19:59에 정상 발화해 전송하면 푸시는 아예 안 나간다** — `last_seen`이 오늘이 되어
  20:00 tick의 쿼리가 그 기기를 결과 집합에서 떨어뜨린다. 오염이 "구분 가능"한 게 아니라
  **구조적으로 발생하지 않는다.**
- **알람 배달 창이 [19:30, 20:30]이라 20:00 이후 배달도 정상 범위다.** 그 경우 푸시가 먼저 창을
  열게 되는데, 그건 손실이 아니라 **데이터 전용 푸시가 temp-power-save를 받는가(§8.5의 미검증
  전제)를 3일 앞당겨 답하는 기회**다.
- **판별자** — 셋 중 아무거나 하나로 갈린다:

| 증거 | 알람 자력 발화 | 새 푸시가 열어 준 것 |
|---|---|---|
| `dumpsys netpolicy \| grep temp-power-save` | 항목 **없음** | `reason=PUSH_MESSAGING` 항목 **있음** |
| `HeartbeatAlarm: FIRED` 시각 | 19:37~19:59 | 20:00:0x (허용 직후) |
| Railway 로그 | `[사일런트 깨우기]` 줄 없음 | `발송 — 대상 1건 / 성공 1건 / Nms` |

⚠️ 잃는 것 하나: **알람이 20:00을 넘겨 뛰었을 경우 그 자연 오프셋을 못 잰다**(§6 상한 60분인데
푸시가 30분에서 끊는다). 수용 가능한 손실로 판단했다.

#### 8.10.4 ★ 09-13 대조군 결과 — appop 해제 → 알람이 **+29분 17초**로 돌아왔다 (예측 적중)

```
09-12 21:40:54.038  ARMED (arm(19:30)) for=09-13 19:30:00          ← 전날 포그라운드 무장
09-13 19:59:17.529  Received BROADCAST … HEARTBEAT_ALARM
09-13 19:59:17.580  Start proc 26180 … for broadcast {HeartbeatAlarmReceiver}   ← FCM 아님
09-13 19:59:17.829  ARM skipped (app-process-start) — 오늘 발화 창 유지 (19:30)
09-13 19:59:17.835  ARMED (refire) for=09-14 19:30:00                ← 재무장 정상
09-13 19:59:17.849  FIRED at=19:59:17.836 idle=true bucket=40        ← 예약 +29m17s
09-13 19:59:28.229  HOLD started budget=90000ms                      (엔진 부팅 약 10초)
09-13 19:59:40.570  HeartbeatSend: OK src=one-off attempt=1 key=2026-09-13_19:30 steps=17
09-13 19:59:40.751  HOLD ended held=12524ms reason=heartbeat-done
```

20:02 판독: `RUN_ANY_IN_BACKGROUND` 여전히 `No operations.`(allow), 버킷 40, `mState=IDLE`,
`background restricted` 킬 로그 **없음**.

**판정**

| 질문 | 답 |
|---|---|
| appop을 풀면 알람이 21:30 고정에서 벗어나는가 | ✅ **벗어났다.** 19:59:17 — 차단 전(09-02 +29m18s)·§7.7 5일치와 같은 오프셋 |
| 전송이 창 안에서 끝났는가 | ✅ `FIRED → HOLD started → src=one-off → HOLD ended heartbeat-done` 4줄 체인(§7 판독법) |
| 20:00 사일런트 깨우기가 나갔는가 | ❌ **안 나갔다(설계대로).** 19:59:40에 `last_seen`이 오늘이 되어 20:00 tick 쿼리에서 제외. uid 11227의 `temp-power-save` 항목 **0건**, 프로세스 시작 사유도 알람 브로드캐스트 1건뿐 |
| 데이터 전용 FCM이 temp-power-save를 받는가 | **미답** — 푸시가 안 나갔으므로 오늘 답할 수 없다 |

**§8.4의 인과 판정이 한 단계 올라간다.** 같은 기기·같은 예약시각에서 appop만 달랐던 날의
A/B다 — `ignore`(09-04~09-12) → 21:30 고정, `allow`(09-13) → +29m17s. 다만 ⚠️ **확정이 아니라
강한 정황**으로 적는다. §8.10.1의 변수 셋 중 버킷은 오히려 40으로 §7.7과 맞아떨어져 교란이
줄었지만, **알람 무장 경로(포그라운드)는 여전히 달랐다.**

⚠️ `src=one-off`이다(§7.7의 성공 체인은 `src=periodic`이었다). 창 안에서 어느 워커가 먼저
락을 잡느냐의 차이일 뿐 판정에는 영향 없다 — §7 판독법대로 4줄을 묶어 읽을 것.

**다음 관측**: 마지막 포그라운드 사용 09-12 21:46 + 약 3일 → **09-15~16 재편입 예상.** 그때까지
앱을 열지 않으면 차단 상태에서 +30m 잡의 첫 실전(데이터 전용 allowlist 판정)을 볼 수 있다.
판정 기준은 §8.10.3 표 그대로 — 기대값은 `HeartbeatSend: OK`가 **20:00:0x**, `netpolicy`에
uid 11227 `reason=PUSH_MESSAGING` 항목. 21:30에 붙으면 데이터 전용은 allowlist를 **받지 못한다**.

#### 8.10.5 +30m 사일런트 깨우기 첫 실전 (09-13 18:30, 실사용자) — 무해 확인, 효과는 **미답**

배포 당일 18:00 예약 사용자들의 +30m tick. Railway 로그 + `heartbeat_logs` 대조.

```
18:03~18:05  안드로이드 18:00 기기 4대 자력 전송 → 대상에서 빠짐
18:30:01     [사일런트 깨우기] 발송 — 대상 3건 / 성공 3건 / 166ms
18:30:01     POST /api/v1/heartbeat 200  ← 4238256… (18:00 예약)
20:00:00     subject_safety_net ×2  ← 나머지 2대 (+2h)
20:00:00     POST /api/v1/heartbeat 200  ← 23fb33… (예약 20:00 — +2h 반응 아님)
20:05까지    나머지 2대 무전송
```

| 대상 | 판정 |
|---|---|
| 4238256… | **판정 불가.** 지난 7일 도착이 18:05~18:36(09-07 18:36)으로 **원래 정상 기기**다 — 20:00에 몰린 날이 없어 앱옵 차단 이력이 없다. 오늘 18:30:01은 평소 범위 안이라 푸시 효과와 구분되지 않는다(같은 초라는 점은 정황일 뿐) |
| 나머지 2대 | **정보 없음.** 20:00 표시형 알림에도 무반응 → 꺼짐·오프라인. 사일런트의 실패 증거가 아니다 |

**확인된 것**: 발송 성공 3/3, 에러 0, 사용자 가시 효과 0. **미확인**: 앱옵 차단 기기에서
데이터 전용 FCM이 temp-power-save를 받는가 — 오늘 대상 중 차단 기기가 없었다.

⚠️ 판독 함정 두 개를 실제로 밟았다:
1. **`POST /heartbeat`의 시각만으로 "푸시에 반응했다"고 읽지 말 것** — 20:00:00 도착은 +2h
   반응처럼 보였으나 **예약 20:00인 다른 기기**였다. 액세스 로그에 device_id가 없으므로
   반드시 `heartbeat_logs`의 `scheduled_key`로 대조한다.
2. **같은 초 도착을 인과로 읽기 전에 그 기기의 평소 도착 분포를 볼 것** — 원래 그 시간대에
   보내던 기기면 판정 불가다. 앱옵 차단 기기는 지난 며칠이 **+2h 직후에 몰린다**(§8.4 서명).

#### 8.10.6 A66-Q1OR (09-14, 실사용자) — 사일런트 시험 대상이었으나 **판정 불가**

차단 서명이 뚜렷한 실사용자 기기가 +30m 사일런트 대상에 들어왔다.

| 날짜 | 도착 | 해석 |
|---|---|---|
| 09-06 ~ 09-10 | **20:00:12 ~ 14** (5일 연속) | 예약 18:00. **+2h 표시형 알림 12~14초 뒤**에만 전송 = §8.4의 차단 서명 |
| 09-11 | 22:37 | 앱 실행으로 해제된 것으로 보임 |
| 09-12 | 19:31 | |
| 09-13 | 18:05 | 정상 |
| **09-14** | **없음** | 09-11 +3일 → 재편입 추정 |

```
09-14 18:30:05  [사일런트 깨우기] 발송 — 대상 3건 / 성공 3건 / 261ms   ← 이 기기 포함(조건 전부 일치)
18:30 ~ 19:59   무전송
20:00:00        subject_safety_net (cG7WPprpR7...) 발송 완료              ← +2h 표시형
                CAUTION 대상자=A66-Q1OR ×5 (보호자)
20:00 ~ 20:05   ★ 무전송 — 표시형에도 반응 없음
```

**판정: 불가.** 5일 연속 +2h 표시형에 12~14초 만에 반응하던 기기가 **그날은 표시형에도 반응하지
않았다** → 그날 기기에 도달할 수 없었다(꺼짐·망 단절·방전 등). 따라서 18:30 사일런트 무반응도 같은
이유일 수 있어 **사일런트의 실패 증거가 아니다.**

⚠️ 판독 함정 — **"한 번도 실패한 적 없는 기기"라는 이력만으로 오늘의 무반응을 푸시 종류 탓으로 읽지 말 것.**
18:30 시점에 "사실상 부정"으로 적었다가 20:00 결과로 뒤집혔다. **같은 날 대조 푸시(표시형)의 반응을
확인하기 전에는 판정하지 않는다.**

→ 그래도 같은 날 서버를 **+60m 표시형 조기 미전송 알림**으로 교체했다(§8.5 대응). 근거는 이 관측이
아니라 **FCM 문서의 강등 규칙 + 표시형이 문을 연다는 실측(§8.9)**이다.

부수 확인: 사일런트 성공 로그에는 기기 식별자가 없어 "대상 3건이 누구인지"를 로그로 알 수 없었다.
표시형은 `send_push`가 `subject_safety_net (토큰 앞 10자)`를 남긴다 — 이날 20:00의 3건
(`fyOOqMIERg`/`fjvFRmtSQ-`/`cG7WPprpR7`)이 18:30 대상 3건과 같은 기기로 보이며, 앞 두 기기는
09-13에도 무반응이라 **장기 도달 불가 기기**일 가능성이 크다.
