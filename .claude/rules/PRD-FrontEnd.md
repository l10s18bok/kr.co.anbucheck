...
| 안전망 (3차)          | **periodic 15분 폴링** (WorkManager)이 1차 one-off 누락 시 최대 15분 내 백업 발화. 최종적으로 앱 열기 자동 전송(2차)이 보완 | **오늘의 안부 확인 메시지 로컬 알림** (heartbeat 예약 시각, 매일 반복) — BGTask 실패 시 사용자에게 앱 실행 유도  |
...
[3차 (iOS 전용): 로컬 알림 오늘의 안부 확인 메시지 로컬 알림 — 1차·2차 모두 실패 시]
    ...
    │   ※ Android는 오늘의 안부 확인 메시지 로컬 알림 없이 WorkManager periodic 15분 폴링과 포그라운드 복귀 자동 전송(2차)이 안전망 역할을 한다
    │
...
- **Android Doze/OEM 절전 모드**: 스와이프 종료 + 화면 꺼짐 상태에서 one-off 태스크가 지연/미실행될 수 있음. periodic 15분 폴링이 최대 15분 내 백업 발화로 대부분 복구하며, 그래도 실패 시 앱 열기 자동 전송(2차)이 최종 안전망. 사전 안내 다이얼로그는 두지 않고 사용자가 OEM 정책에 따라 직접 조정하도록 맡긴다 (Google Play 정책상 `REQUEST_IGNORE_BATTERY_OPTIMIZATIONS` 권한 미사용).
