...
| 계층 | 방식 | 실행 조건 | 특성 |
|------|------|-----------|------|
| 1차 (정확) | WorkManager `registerOneOffTask` (Android) | 예약 시각 정각 도래 | 정확한 발화. 전송 성공 후 `HeartbeatService._onHeartbeatSent`가 `HeartbeatWorkerService.schedule()` 호출 → one-off + periodic 둘 다 cancel + 내일자 register. iOS는 사용하지 않음 |
