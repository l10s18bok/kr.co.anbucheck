...
걸리지 않아 예전처럼 원본만 통과한다. ⚠️ **서버 동반 수정이 선행돼야 한다**: 회복 전송이
활성 경고를 해소하지 않도록 게이팅하지 않으면 경고 등급 사다리가 매일 리셋된다. 상세는
`ios_nse_field_notes.md` §20.
