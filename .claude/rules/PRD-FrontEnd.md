...
5. **확장 전송은 `suspicious=false` 고정이다.** 지금까지 iOS heartbeat는 전부 포그라운드에서
   ...
   조건이 하나 더 있다). 없던 경고 종류를 "탭 없이 전달"이라는 핵심 변경과 같은 릴리스에 넣지
   않기로 했다. 걸음수는 계속 실어 보내므로 실제 분포가 쌓이고, 나중에 켜는 것은 쉽다.
