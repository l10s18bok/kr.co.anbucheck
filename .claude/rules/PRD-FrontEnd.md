...
| 3차 (안전망) | 일일 로컬 안부 확인 안전망 알림 (`LocalAlarmService`) | **iOS**: heartbeat 시각 정시 / **Android**: heartbeat 시각 + 3시간 (매일 반복, daily repeat via `matchDateTimeComponents.time`) | **iOS**: BGTask 미사용이므로 사실상 PRIMARY 트리거 — 사용자가 알림을 탭하면 앱이 열리며 `onInit`/`onResumed`에서 자동 전송. **Android**: WorkManager one-off + periodic 15분 + 앱 열기(2차) 모두 실패해 worker 자체가 OEM/사용자에 의해 cancel된 시나리오까지 메우는 LAST-RESORT 사용자 유도 알림. 자정 넘기면 자연 롤오버(예: 22:30 → 다음 날 01:30). payload 분리: iOS `gs_deadman`(G+S 라우팅), Android `safety_net`(no-op, 앱 진입만). heartbeat 성공 시 `_onHeartbeatSent`가 `schedule(forceNextDay: true)` → `cancel(_alarmId)`로 예약 + 표시 중 알림 모두 제거 + 내일자 재예약 |
...
[3차 (안전망): 일일 로컬 안부 확인 알림 — 1차·2차 모두 실패 시]
    ...
    │   ※ payload 분리: iOS `gs_deadman`(G+S 라우팅) / Android `safety_net`(no-op, 앱 진입만)
    │
...
| 일일 로컬 안전망 알림 터치 (iOS 정시 / Android +3h)        | `main()` → 앱 포그라운드 전환만 (알림 자체에서 heartbeat 전송 안 함) → 홈 화면 `onInit`/`onResumed`에서 예약시각 경과 + 미전송 시 자동 전송. iOS `gs_deadman`은 G+S 라우팅 적용, Android `safety_net`은 추가 라우팅 없음 | 전체 화면 |
...
| iOS 일일 로컬 안전망 알림 (정시) | `gs_deadman` | iOS G+S 라우팅 (Dashboard 위에 SafetyHome push + `refreshAndForceSend`) | iOS는 `LocalAlarmService.alarmPayload` |
| Android 일일 로컬 안전망 알림 (+3h) | `safety_net` | 무시 → 홈 화면 자동 전송 | Android는 별도 라우팅 없음, 컨트롤러 onResumed가 자동 전송 |
| Android retry 실패 알림 (즉시) | `send_failed` | 무시 → 홈 화면 자동 전송 | retry 3회 실패 시 즉시 표시, heartbeat 성공 시 `cancelSendFailed`로 제거 |
