...
Splash → 버전 체크 → 플랫폼 분기
    ...
    │       권한 요청 안내 화면 (모드 선택 후 진입)
    │       ├─ 알림 권한 (필수)
    │       ├─ 신체 활동 권한 (ACTIVITY_RECOGNITION, 모드 무관 항상 표시)
    │       └─ [확인] 탭 → OS 권한 팝업 순차 표시
    ...
    └─ [iOS] 모드 선택 스킵 → 보호자 모드 직행
        ...
        ├─ 알림 권한 (필수, APNs)
        ├─ 모션/피트니스 권한 (NSMotionUsageDescription, 추후 G+S 활성화 대비)
        └─ [확인] 탭 → OS 권한 팝업 순차 표시
        Onboarding → 서버 등록 (자동) → 대시보드 (보호 대상자 추가 대기)
```
...
| 트리거                | 2계층: `registerOneOffTask()` 정확 시각 + `registerPeriodicTask(frequency: 1h)` 안전망 폴링. one-off는 fire 후 내일로 재등록, periodic은 재등록 없이 유지(UPDATE는 initialDelay 무시, REPLACE는 self-cancel). race는 2중 dedup(`lastHeartbeatDate` + `lastScheduledKey`)으로 차단 | `registerProcessingTask()` → BGProcessingTask (earliestBeginDate 존중). ⚠️ `registerOneOffTask()`는 iOS에서 `beginBackgroundTask`를 사용하여 즉시 실행되므로 사용 금지 (flutter_workmanager PR #511) |
...
| 안전망 (3차)          | **periodic 1시간 폴링** (WorkManager)이 1차 one-off 누락 시 최대 1시간 내 백업 발화. 최종적으로 앱 열기 자동 전송(2차)이 보완 | **로컬 반복 알림 데드맨 스위치** (heartbeat 예약 시각, 매일 반복) — BGTask 실패 시 사용자에게 앱 실행 유도  |
| 앱 강제 종료 시       | WorkManager는 OS가 재스케줄링하므로 **정상 동작** (단, 정확한 시각 보장 불가)                                      | BGTask 미실행 → 데드맨 스위치 로컬 알림으로 사용자 유도                                            |
...
- BGTaskScheduler(iOS)는 정확한 시각 보장이 안 되지만, 앱 열기 자동 전송 + 데드맨 스위치와 조합하면 충분한 신뢰성 확보
...
| 1차 (폴링 안전망) | WorkManager `registerPeriodicTask(frequency: 1h)` (Android) | 매 1시간 | one-off가 OEM 배터리 절약/Doze로 지연·미실행되어도 최대 1시간 내 백업 발화. fire 후 재등록하지 않고 유지(UPDATE/REPLACE 모두 이슈 있음). 당일 중복 전송은 2중 dedup으로 차단 |
| 2차 | 앱 열기/복귀 자동 전송 | 예약 시각 경과 + 당일 미전송 | 포그라운드 진입 시 즉시 전송. 1차 2계층 모두 실패 시 최종 안전망 |
| 3차 (iOS) | 로컬 알림 데드맨 스위치 | heartbeat 시각 + 30분 (매일 반복, iOS 전용) | BGTask 미사용이므로 iOS는 이것이 1차. 사용자가 알림을 탭하면 앱이 열리며 `onInit`/`onResumed`에서 자동 전송. Android는 데드맨 알림 없음 |
...
[1차: WorkManager/BGTaskScheduler — 백그라운드 실행]
    ...
    │       ※ 두 계층이 거의 동시에 fire되는 race는 아래 dedup 2중 방어선으로 차단:
    │          (1) 콜백 진입 시 lastHeartbeatDate == 오늘 검사
    │          (2) HeartbeatService._executeInternal의 lastScheduledKey 검사
    │
    ├─ iOS:
...
[3차 (iOS 전용): 로컬 알림 데드맨 스위치 — 1차·2차 모두 실패 시]
    │
    ...
    │   └──────────────────────────────────────┘
    │   ※ 기본: heartbeat 09:30 → 데드맨 알림 10:00
    │   ※ repeats: true → 사용자가 앱을 열 때까지 매일 같은 시각에 반복
    │   ※ Android는 데드맨 알림 없이 WorkManager periodic 1시간 폴링과 포그라운드 복귀 자동 전송(2차)이 안전망 역할을 한다
    │
    └─ 사용자가 알림 탭 → 앱 실행 → 2차 로직에 의해 heartbeat 전송

[정상 동작 시]
    매일 09:30 WorkManager/BGTask 실행 → heartbeat 성공 → 예약시각 매일 재예약
    → 데드맨 알림(10:00)이 실제로 표시될 일 없음
...
- 보호 대상자 모드 선택 → 서버 등록 완료 시점에:
  ...
  2. iOS: 데드맨 스위치 로컬 알림 등록 (heartbeat 시각 + 30분, repeats: true)
  3. 첫 heartbeat 즉시 전송
...
[보호 대상자 모드 최초 등록 시]
    ...
    ├─ WorkManager/BGTask 예약 (다음 날 09:30)
    ├─ iOS: 데드맨 스위치 매일 반복 알림 등록 (기본 10:00, repeats: true)
...
- iOS: 사용자가 알림 권한을 거부하면 3차 안전망(데드맨 스위치) 동작 안 함 → 모드 선택 후 권한 요청 안내 화면(9.0)에서 중요성 안내
...
- 지정 시각 + 2시간 내 heartbeat 미수신 → 경고 발생 (기본: 09:30 미수신 → 11:30 "주의" -> 2번째 "경고" -> 3번이상 "긴급")
...
- suspicious = ture, 1회 "주의" -> 2번째 "경고" -> 3번이상 "긴급" → 순서로 보호자에게 알림 발송
- 보호자가 판단하여 직접 확인하도록 유도

**권한:**
- Android: `ACTIVITY_RECOGNITION` 런타임 권한 필요 — 권한 요청 안내 화면에서 `Permission.activityRecognition.request()` 호출 시 시스템 팝업 표시
- iOS: `NSMotionUsageDescription` Info.plist 등록 필수. **iOS는 `Permission.activityRecognition.request()` 호출만으로는 시스템 팝업이 뜨지 않고**, CMPedometer/CMMotionActivityManager 데이터를 실제로 조회하는 시점에만 최초 1회 팝업이 표시됨. 따라서 권한 요청 안내 화면에서 `Pedometer.stepCountStream.first.timeout(3s)`를 호출해 CMPedometer를 유발 → 시스템 팝업 표시
- `ios/Podfile`에 `PERMISSION_SENSORS=1` 매크로 추가 필요 (permission_handler의 권한 상태 조회용)
...
| WorkManager/BGTask (매일 예약 시각)              | 센서 스냅샷(가속도+자이로) 조회 → heartbeat 전송 → 다음날부터 해당 시각 매일 재예약(전송당일 예약x) | 없음      |
| 데드맨 스위치 로컬 알림 터치 (iOS 대상자)        | `main()` → 앱 포그라운드 전환만 (알림 자체에서 heartbeat 전송 안 함) → 홈 화면 `onInit`/`onResumed`에서 예약시각 경과 + 미전송 시 자동 전송 | 전체 화면 |
...
Day 1, 11:30: 서버 확인 (09:30 + 2시간) →  "주의" (보호자에게 Push 알림)
Day 2, 09:30: heartbeat 미수신
Day 2, 11:30: "경고" (보호자에게 Push 알림)
Day 3, 11:30: "긴급" (보호자에게 Push 알림)
Day 4, 11:30: "긴급" (보호자에게 Push 알림)
Day 5, 11:30: "긴급" (보호자에게 Push 알림)
Day 6, 11:30: "긴급" (보호자에게 Push 알림)
Day 7, 11:30: "긴급" (보호자에게 Push 알림)
...
- 주의,경고,긴급 알림은 heartbeat 시각 + 2시간에 발송되므로, 기본값(09:30) 기준 **11:30에 보호자 알림** → 보호자가 당일 오후에 대응 가능
...
| 상황                              | 대응                                                                                                                                                                                                                                                                                                       |
| --------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| iOS 앱 스와이프 강제 종료         | BGTask 미실행 → 데드맨 스위치 로컬 알림(10:00)으로 사용자에게 앱 실행 유도 → 무시 시 서버가 미수신 감지 → 보호자에게 경고                                                                                            
| 네트워크 장시간 불가 (Android)    | WorkManager 실행 → 네트워크 없음 감지 → 로컬 큐에 적재 + 대상자 폰에 로컬 알림 표시 ("인터넷 연결이 꺼져 있습니다") + 복구 시 일괄 전송 → 서버가 경고 자동 해소                                                                                                                                            |
| 네트워크 장시간 불가 (iOS)        | BGTask 실행 실패 또는 네트워크 없음 → heartbeat 시각 + 30분 후 **데드맨 스위치 로컬 알림** 표시 ("안부 확인이 필요합니다. 이 알림을 터치 해주세요") → 사용자가 알림 탭 → 앱 실행 → 네트워크 복구 시 heartbeat 전송 → 서버가 경고 자동 해소. 알림 무시 시 매일 반복 + 서버 경고 플로우로 전환                          |
...
| `flutter_local_notifications` | 로컬 알림 예약/취소 | 데드맨 스위치 + 배터리/네트워크 안내 알림 (Android/iOS 공통) |
| `pedometer` | 걸음수 조회 | **primary 활동 지표**, Android: ACTIVITY_RECOGNITION 권한, iOS: NSMotionUsageDescription |
...
│                             │
│  ┌───────────────────────┐  │
│  │ 🚶 신체 활동 권한      │  │  ← Android/iOS 모두 표시 (모드 무관)
│  │  걸음수를 감지하여      │  │
│  │  활동 여부를 확인하는   │  │
│  │  데 사용됩니다          │  │
│  └───────────────────────┘  │
...
- 신체 활동 권한 카드는 **Android/iOS 모두, 모드와 무관하게 항상 표시** (보호자/대상자/G+S 동일). iOS는 추후 G+S 활성화 시 pedometer 동작을 위해 최초 설치/재설치 시점에 미리 요청
- [확인] 탭 시 OS 권한 팝업 순차 표시:
  - iOS: ① Firebase Messaging `requestPermission()`으로 APNs 권한 + FCM 토큰 발급 → ② `Pedometer.stepCountStream.first.timeout(3s)` 호출로 CMPedometer를 유발해 모션 권한 시스템 팝업 표시
  - **iOS 특이사항**: `Permission.activityRecognition.request()`는 iOS에서 시스템 팝업을 띄우지 않음. CMPedometer/CMMotionActivityManager 데이터 조회 시점에만 최초 1회 팝업이 뜨므로, Dart 측에서 pedometer stream을 한 번 구독해 트리거한다. `ios/Podfile`에 `PERMISSION_SENSORS=1` 매크로 필요 (권한 상태 조회용)
  - Android: `permission_handler`로 알림 권한 → `Permission.activityRecognition` 순차 요청 (사전 안내 다이얼로그 없이 OS 팝업 직접 표시)
  - iOS 모션 권한 팝업 문구는 `ios/Runner/{lang}.lproj/InfoPlist.strings` 20개 언어로 번역되어 기기 locale에 맞게 자동 표시
...
- **활성화** (`GuardianDashboardController.enableSubjectFeature()`):
  ...
  2. 로컬 `isAlsoSubject=true`, invite_code, heartbeat 스케줄 저장
  3. 예약:
  ...
  4. 첫 heartbeat 즉시 전송 (`HeartbeatService().execute()`) — 이후 SafetyCode `onInit`의 자동 재전송은 `isReportedToday=true`로 dedup되어 중복 전송되지 않음
  5. 성공 시 `goToSafetyCode()` 호출 → 안전코드 페이지(`GuardianSafetyCodePage`)로 자동 이동
  ※ 활동 인식 권한은 최초 권한 화면(9.0)에서 이미 요청되었으므로 별도 요청 없음
  ※ iOS는 활성화 전 경고 다이얼로그를 반드시 표시 — "매일 정해진 시각의 안부 푸시 알림을 탭하거나 그 전후로 앱을 열어야 안부가 전송됨" 안내 후 `이해했습니다, 활성화` 버튼으로만 확정
...
iOS는 BGTaskScheduler의 불안정성 때문에 백그라운드 예약 실행을 사용하지 않는다. 대신 `LocalAlarmService`가 **heartbeat 예약 시각에 정확히** 데드맨 로컬 알림(payload `gs_deadman`)을 표시하고, 사용자가 알림을 탭하거나 앱을 직접 열었을 때 홈 화면의 `onInit`/`onResumed`에서 미전송 여부만 확인하여 즉시 heartbeat를 전송한다.
...
- **전송 조건**: `isReportedToday == false` 하나뿐. 시각 경과 여부는 확인하지 않음.
  ...
  - G+S 전용 자동 재전송은 `GuardianSafetyCodeController._checkAndSendHeartbeat`가 단독 소유. `onInit` / `onResumed` / FcmService의 `refreshAndSend()` 모두 이 단일 진입점을 호출. Dashboard 컨트롤러는 해당 로직을 갖지 않아 중복 전송이 구조적으로 불가능
- **데드맨 알림 시각**: iOS G+S는 heartbeat 예약 시각 +30분이 아니라 **예약 시각과 동일** (`LocalAlarmService.schedule`에서 오프셋 제거)
- **데드맨 알림 탭 라우팅** (`fcm_service._handleNotificationTap`): payload `gs_deadman` 수신 시 `Get.offAllNamed(guardianDashboard)` + `Get.toNamed(guardianSafetyCode)` — 보호자 대시보드 스택 위에 안전코드 페이지를 push하여, 뒤로가기 시 자연스럽게 대시보드로 복귀. kill 상태 런치에서는 스택에 Dashboard가 없어 `offNamedUntil` predicate가 매칭되지 않고 SafetyCode가 root가 되어 뒤로가기 불가 이슈가 있었기 때문에 `offAllNamed`로 스택을 재구성한다. Dashboard 바인딩이 `permanent: true`이므로 `offAllNamed` 호출 시에도 컨트롤러 재생성 race 없음. 이미 `guardianSafetyCode`에 있는 경우에는 스택을 유지하고 `GuardianSafetyCodeController.refreshAndSend()`를 호출하여 즉시 미전송 heartbeat를 재확인
...
> iOS는 보호자 모드만 지원하며 BGTaskScheduler를 사용하지 않으므로 `fetch`, `processing`, `BGTaskSchedulerPermittedIdentifiers`는 불필요하다. 다만 G+S 활성화 시 `pedometer`로 걸음수를 조회해야 하므로 `NSMotionUsageDescription`은 필수이며, 최초 권한 요청 화면에서 미리 받아둔다.
...
> - 알림 권한(UNAuthorizationOptions) + 모션 권한(`NSMotionUsageDescription`)을 권한 요청 안내 화면(9.0)에서 순차 요청 — `permission_handler`의 `Permission.activityRecognition`이 iOS에서 모션 권한으로 자동 매핑됨
> - 시스템 팝업 문구는 `ios/Runner/{lang}.lproj/InfoPlist.strings` 20개 언어 파일에 `NSMotionUsageDescription` 키로 등록되어 기기 locale에 맞춰 자동 번역 표시 (기본 폴백은 `Info.plist`의 한국어 문자열)
...
```
[1차: WorkManager(Android 2계층) / BGTaskScheduler(iOS) — 매일 예약 시각 (기본 09:30)]
    │ Android: one-off 정각 발화 + periodic 1h 안전망 폴링 (두 계층 동시 등록)
    │
    ├─ 역할 확인 (대상자 모드만 실행)
    ├─ 예약 시각 이전이면 스킵 (periodic 폴링이 예약시각 전에 fire되는 경우)
    ├─ 당일 이미 전송 여부 확인 (lastHeartbeatDate == 오늘 → 스킵, race 1차 방어선)
    ├─ HeartbeatService 내부 lastScheduledKey 검사 (race 2차 방어선)
    │
    ├─ 센서 스냅샷 (sensors_plus)
    ...
    │   │   └─ 전송 성공 시:
    │   │       ├─ lastHeartbeatDate + lastScheduledKey 갱신
    │   │       ├─ 데드맨 스위치 반복 알림 취소+재등록
...
[3차 (iOS 전용): 데드맨 스위치 로컬 알림 — heartbeat 시각 + 30분 (1차·2차 실패 시)]
    │
...
| 알림 | 캐시 키 | 한국어 기본값 |
|---|---|---|
| 데드맨 스위치 (iOS 전용) | `local_alarm_title`, `local_alarm_body` | 📱 안부 확인이 필요합니다 / 이 메시지 알림을 한 번 터치해 주세요. |
