...
#### 2.2.3 기기 시간대 동기화 — 해외 여행·이주·출장 (2026-09-25)

서버 `devices.timezone`은 원래 **가입할 때 한 번만** 저장됐다. 폰은 **지금 있는 곳의 현지 시각**으로
동작하므로(WorkManager 예약, `scheduled_key`의 날짜·시각, iOS 확장) 대상자가 해외로 가면 서버와
폰의 "오늘"·"예약시각"이 어긋났다. 서쪽 이동은 서버 미수신 체크(+2h)가 현지 전송보다 먼저 돌아
매일 미수신 판정이 나고, 큰 시차의 서쪽 이동은 현지 18:00 전송이 서울 기준 다음 날 도착이라
**지난 기록 보정으로 오분류**됐다. 여행은 귀국하면 풀리지만 **이주는 영구히** 남는다.

**규칙**
- **`heartbeat_hour/minute`의 의미는 "현재 있는 곳의 현지 시각"이다.** 이동해도 바꾸지 않는다(폰 동작과 일치).
- 클라가 **현재 IANA 시간대**를 서버에 싣고, 서버는 저장값과 다르고 유효할 때만 갱신한다(서버 규칙은 PRD-BackEnd §4.6 "기기 시간대 동기화").

| 경로 | 싣는 곳 | 역할 |
|---|---|---|
| heartbeat (Dart) | `HeartbeatRemoteDatasource.send()`가 **전송 직전** 병합 | 대상자 — 모든 Dart 전송(정시·회복·보류 큐·수동)이 이 메서드를 지난다 |
| heartbeat (iOS 확장) | `NotificationService.send()`·`sendBackfill()` 딕셔너리 | 앱 없이 전송하는 iOS 주 경로. 확장은 실행마다 새 프로세스라 값이 낡지 않는다 |
| `PUT /devices/fcm-token` | `DeviceRemoteDatasource.updateFcmToken` | 콜드 스타트마다 + 포그라운드 복귀 시 시간대가 바뀌었을 때(`FcmService._syncTimezoneIfChanged`). **보호자 기기**의 방해금지(DND) 시간대를 갱신하는 유일한 경로 |

⚠️ **되돌리지 말 것 — 불변 규칙 3가지**
1. **시간대는 `HeartbeatRequest.toJson()`에 넣지 않는다.** 보류 큐가 `toJson()` 결과를 그대로 저장하므로,
   거기에 넣으면 서울에서 저장된 지난 기록이 파리에서 재전송될 때 서버 값을 **서울로 되돌린다.**
   서버는 이를 막을 수 없다 — 지난 기록인지 판정하려면 새 시간대가 먼저 필요하기 때문이다.
2. **OS 조회에 성공한 원본 문자열만 보낸다**(`DeviceTimezone.current`). 스플래시·워커는 조회에 실패하면
   `tzlib.local`을 `Asia/Seoul`로 폴백하는데, 그 값을 보내면 한국 밖 사용자의 올바른 시간대가 서울로
   덮인다. 그래서 `tzlib.local.name`을 쓰지 않는다. 값이 없으면(조회 실패·FCM 백그라운드 isolate)
   필드를 싣지 않는다 — 서버가 저장값을 유지하는 안전한 방향이다.
3. **포그라운드 복귀에서 `tzlib.setLocalLocation`은 바꾸지 않는다** — 로컬 알림 예약 동작까지 바뀌어
   이 변경의 범위를 벗어난다. `DeviceTimezone.refresh()`는 서버 전송용 값만 갱신한다.

**알려진 동작·한계**
- **이동한 날 하루는 판정이 어긋날 수 있다**(이미 돈 체크는 되돌리지 않는다). 다음 날부터 정상이다.
- **서쪽 이동은 1~2일 미전송 뒤 자가 복구된다.** 예: 서울→LA. 서버 트리거(iOS)·미전송 알림(Android 앱옵 차단)이
  옛 시간대 기준으로 LA 새벽에 와서 "예약시각 전"으로 막힌다. 2일 갭이 생기면 **회복 전송**
  (iOS: 갭 + 사람 흔적 / Android 워커: 갭만)이 나가고 거기 실린 시간대로 서버가 갱신돼 그다음 날부터
  현지 18시에 트리거가 온다. 앱을 열면 즉시 복구된다(`_syncTimezoneIfChanged`).
  **앱옵 차단이 없는 Android**는 이렇게 복구된다(코드 확인 2026-09-25): 이동 **전에** 등록된 one-off
  (`initialDelay` = 등록 시점 기준 **상대 지연**)과 0차 알람(**RTC 절대 시각**)은 옛 시각에 한 번
  발화한다(서울 18시 = LA 새벽 2시) → 예약시각 −15분 이전이라 전송은 건너뛴다. 대신 그 콜백
  진입부의 재무장(`rescheduleOneOffOnly`)과 알람 `onReceive`의 재무장이 **현지 시각으로 다시 계산**
  되고, 살아 있는 periodic 폴링이 현지 예약시각 이후에 전송하면서 시간대가 갱신된다. 즉 대부분
  **그날 안에** 복구되지만, "첫 발화가 곧 정시 전송"은 아니다.
  → 확장이 "예약시각 전"에 시간대만 따로 보내는 전용 경로는 **의도적으로 만들지 않았다** — iOS 주 전송
  경로(20초 예산, 피기백)에 새 네트워크 호출이 끼어드는 위험 대비 이득이 1~2일 단축뿐이다.
- 기기가 `US/Pacific`·`Japan`·`ROK` 같은 **legacy alias**를 반환하면 Railway PostgreSQL에 없는 이름이라
  서버가 갱신하지 않고 옛 값을 유지한다(PRD-BackEnd §4.6).
- 보호자 방해금지(DND)는 **보호자 기기 시간대**로만 판정한다. 대상자가 어디로 이동해도 보호자 DND에는
  영향이 없다.
- 계측: `HeartbeatSend` 로그에 `tz=` 필드, 서버 Railway 로그에 `[tz change] old→new`.
...
void heartbeatWorkerCallback() {
  Workmanager().executeTask((taskName, inputData) async {
    ...
    //      오늘도 어제도 아님)이면 회복 전송(화면 켜짐 조건 없음 — 코드 확인 2026-09-25):
