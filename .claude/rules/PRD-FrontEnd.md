...
| 안전망 (3차)          | **로컬 반복 알림 데드맨 스위치** (heartbeat 시각 + 10분, 매일 반복) — WorkManager 실패 시 사용자에게 앱 실행 유도   | **로컬 반복 알림 데드맨 스위치** (heartbeat 시각 + 10분, 매일 반복) — BGTask 실패 시 사용자에게 앱 실행 유도  |
...
| 3차 | 로컬 알림 데드맨 스위치 | heartbeat 시각 + 10분 (매일 반복) | 1차·2차 모두 실패 시 사용자에게 앱 실행 유도 |
...
[3차: 로컬 알림 데드맨 스위치 — 1차·2차 모두 실패 시]
    │
    ├─ heartbeat 시각 + 10분에 매일 반복 로컬 알림
    │   ┌──────────────────────────────────────┐
    │   │ 📱 안부 확인이 필요합니다              │
    │   │                                      │
    │   │ 이 메시지 알림을 한 번 터치해 주세요.   │
    │   └──────────────────────────────────────┘
    │   ※ 기본: heartbeat 09:30 → 데드맨 알림 09:40
...
- 보호 대상자 모드 선택 → 서버 등록 완료 시점에:
  ...
  2. 데드맨 스위치 로컬 알림 등록 (heartbeat 시각 + 10분, repeats: true)
  3. 첫 heartbeat 즉시 전송
...
[보호 대상자 모드 최초 등록 시]
    ...
    ├─ 데드맨 스위치 매일 반복 알림 등록 (기본 09:40, repeats: true)
...
- 사용자가 알림 권한을 거부하면 3차 안전망 동작 안 함 → 모드 선택 전 권한 요청 안내 화면(7.0)에서 중요성 안내
...
| 데드맨 스위치 로컬 알림 터치 (대상자)            | `main()` → heartbeat 자동 전송(위와 동일)                   | 전체 화면 |
...
| 상황                              | 대응                                                                                                                                                                                                                                                                                                       |
| --------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| iOS 앱 스와이프 강제 종료         | BGTask 미실행 → 데드맨 스위치 로컬 알림(09:40)으로 사용자에게 앱 실행 유도 → 무시 시 서버가 미수신 감지 → 보호자에게 경고                                                                                            
| 네트워크 장시간 불가 (Android)    | WorkManager 실행 → 네트워크 없음 감지 → 로컬 큐에 적재 + 대상자 폰에 로컬 알림 표시 ("인터넷 연결이 꺼져 있습니다") + 복구 시 일괄 전송 → 서버가 경고 자동 해소                                                                                                                                            |
| 네트워크 장시간 불가 (iOS)        | BGTask 실행 실패 또는 네트워크 없음 → heartbeat 시각 + 10분 후 **데드맨 스위치 로컬 알림** 표시 ("안부 확인이 필요합니다. 이 알림을 터치 해주세요") → 사용자가 알림 탭 → 앱 실행 → 네트워크 복구 시 heartbeat 전송 → 서버가 경고 자동 해소. 알림 무시 시 매일 반복 + 서버 경고 플로우로 전환                          |
...
| `go_router`                                | 라우팅                 |                                                                  |
| `firebase_messaging`                       | FCM/APNs Push 수신     | 보호자 경고 Push만 (Silent Push 미사용)                           |
...
└─────────────────────────────┘
```
- [확인] 탭 시 OS 권한 팝업 순차 표시 (알림 권한 → 활동 인식 → 배터리 최적화 제외)
- 배터리 최적화 제외는 Xiaomi, Huawei, OPPO 등 중국산 OEM 기기에서만 표시 (삼성·구글 Pixel 등 제외)
...
[3차: 데드맨 스위치 로컬 알림 — heartbeat 시각 + 10분 (1차·2차 실패 시)]
    │
    ├─ 매일 반복 로컬 알림 표시 ("안부 확인이 필요합니다. 앱을 열어주세요.")
    ├─ 사용자가 알림 탭 → 앱 실행 → 2차 로직에 의해 heartbeat 전송
    └─ 무시 시 매일 같은 시각에 계속 반복
