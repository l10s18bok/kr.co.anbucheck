...
```
- `steps_delta`:
  - 자동 heartbeat → **오늘 자정 ~ 현재 시각** 누적 걸음수 (권한 허용 시), 거부 시 null
  - 수동 heartbeat (`manual=true`) → **항상 null 전송** — 서버의 활동 정보 알림 추가 생성을 클라이언트가 차단 (서버 가드: `steps_delta is not None and > 0`)
- `suspicious`: 앱에서 판정 후 전송 (steps_delta > 0이면 항상 false, 수동 보고는 항상 false)

**필요 패키지:** `pedometer_2` (걸음수, iOS `queryPedometerData` + Android Google Fit Local Recording API 통합), `sensors_plus` (가속도/자이로), `battery_plus` (배터리 잔량, 권한 불필요)
...
| health (활동 정보) | ❌ | ✅ | 활동 정보 알림 — 오늘 자정~현재 누적 걸음수 표시 ("오늘 N보를 걸으셨습니다", `steps_delta > 0` 일 때만 생성, 수동 보고는 항상 null이라 미생성) |
...
- **Lazy Permission 원칙** — 걸음수(신체 활동/모션) 권한은 "걸음수 데이터 전송이 실제로 필요한 시점"까지 요청을 지연한다:
  - **표시 조건**: `Platform.isAndroid && (isSubjectMode || isAlsoSubject)` 일 때만 신체 활동 권한 카드 및 OS 권한 요청을 수행
  - **숨김 조건**: iOS 전체(보호자 전용이므로 heartbeat 전송 없음) + Android 순수 보호자 모드 (heartbeat 전송 없음)
  - iOS G+S 활성화 시점에 별도 플로우로 모션 권한을 요청 (§9.7 참조) — App Store 심사 시 "선택 기능 활성화 시에만 모션 권한 요청" 포지셔닝 유지
  - G+S 재설치 복원(`isAlsoSubject=true`)은 이미 해당 기능을 사용 중인 사용자이므로 Android 한정으로 최초 권한 화면에서 미리 요청
- [확인] 탭 시 OS 권한 팝업 순차 표시:
  - iOS: Firebase Messaging `requestPermission()`으로 APNs 권한 + FCM 토큰 발급 (모션 권한 요청 없음)
  - Android 대상자/G+S 복원: `permission_handler`로 알림 권한 → `Permission.activityRecognition` 순차 요청 (사전 안내 다이얼로그 없이 OS 팝업 직접 표시)
  - Android 순수 보호자: 알림 권한만 요청 (활동 인식 권한 요청 없음)
...
- "지금 안부 보고하기" 버튼으로 수동 heartbeat 즉시 전송 가능 (manual = true) + 연락처 선택 → 전화 걸기
  ...
  - 수동 보고 시 `stepsDelta=null` 강제 전송 → 보호자 알림 목록에 "수동 안부 확인"(`manual_report`) 1건만 도달, 활동 정보 알림(`steps`)은 미생성
...
- **활성화** (`GuardianDashboardController.enableSubjectFeature()`):
  ...
  6. 첫 heartbeat 즉시 전송 (`HeartbeatService().execute()`) — 이후 SafetyCode `onInit`의 자동 재전송은 `HeartbeatService` 내부 `heartbeat_in_flight`(30s TTL mutex) + `lastScheduledKey`(성공 마커) + `isReportedToday=true`로 중복 전송이 구조적으로 차단됨
...
> iOS는 보호자 모드만 지원하며 BGTaskScheduler를 사용하지 않으므로 `fetch`, `processing`, `BGTaskSchedulerPermittedIdentifiers`는 불필요하다. G+S 활성화 시 `pedometer_2`로 걸음수를 조회해야 하므로 `NSMotionUsageDescription` 선언은 필수이나, **시스템 팝업은 G+S 활성화 시점에만 표시된다** (Lazy Permission — App Store 심사 시 "선택 기능 활성화 시에만 요청" 포지셔닝 유지).
...
> - 최초 권한 요청 화면(§9.0): 알림 권한(UNAuthorizationOptions)만 요청 — 모션 권한은 요청하지 않음
