...
| 트리거                | 2계층: `registerOneOffTask()` 정확 시각 + `registerPeriodicTask(frequency: 1h)` 안전망 폴링. one-off는 fire 후 내일로 재등록, periodic은 재등록 없이 유지(UPDATE는 initialDelay 무시, REPLACE는 self-cancel). race 방어는 **역할 분리된 2선**: (1) `lastScheduledKey` = 성공 마커(API 전송 성공 + `lastHeartbeatDate` 저장 후에만 save, 당일 재전송 차단), (2) `heartbeat_in_flight` = **30초 TTL mutex lock**(센서/전송 시작 직전 now ms save, finally에서 clear, TTL 초과 시 crashed isolate로 간주하고 이어받음) | `registerProcessingTask()` → BGProcessingTask (earliestBeginDate 존중). ⚠️ `registerOneOffTask()`는 iOS에서 `beginBackgroundTask`를 사용하여 즉시 실행되므로 사용 금지 (flutter_workmanager PR #511) |
...
| 1차 (폴링 안전망) | WorkManager `registerPeriodicTask(frequency: 1h)` (Android) | 매 1시간 | one-off가 OEM 배터리 절약/Doze로 지연·미실행되어도 최대 1시간 내 백업 발화. fire 후 재등록하지 않고 유지(UPDATE/REPLACE 모두 이슈 있음). 당일 중복 전송은 `lastScheduledKey`(성공 마커) + `heartbeat_in_flight`(30s TTL mutex)로 차단 |
...
[1차: WorkManager/BGTaskScheduler — 백그라운드 실행]
    ...
    │       ※ 두 계층이 거의 동시에 fire되는 race는 역할 분리된 2선 방어로 차단:
    ...
    │              · heartbeat_in_flight(30s TTL mutex) 검사 — 다른 isolate 전송 중이면 스킵
    │              · 센서/전송 시작 직전 heartbeat_in_flight save → finally에서 clear
    │              · TTL 30s 초과 시 crashed isolate로 간주하고 이어받음 (ghost state 방지)
    │          ※ 과거에는 lastScheduledKey를 선점 save해 락과 성공 마커를 겸용했으나,
    │            Worker가 Doze/OEM 절전으로 중도 종료되면 선점 마커만 남아 2차 안전망
    │            (앱 복귀 자동 전송)이 영구 차단되는 ghost state 버그가 있어 역할을 분리함
    │
    ├─ iOS:
...
[3차 (iOS 전용): 로컬 알림 오늘의 안부 확인 메시지 로컬 알림 — 1차·2차 모두 실패 시]
    ...
    │   └──────────────────────────────────────┘
    │   ※ 기본: heartbeat 09:30 → 오늘의 안부 확인 메시지 로컬 알림 10:00
...
[정상 동작 시]
    ...
    → 오늘의 안부 확인 메시지 로컬 알림(10:00)이 실제로 표시될 일 없음

[비정상 시 (BGTask 미실행, 앱 미사용) — iOS]
    당일 10:00 → 오늘의 안부 확인 메시지 로컬 알림 표시
...
[보호 대상자 모드 최초 등록 시]
    ...
    ├─ iOS: 오늘의 안부 확인 메시지 로컬 알림 (매일 반복) 등록 (기본 10:00, repeats: true)
...
| 상황                              | 대응                                                                                                                                                                                                                                                                                                       |
| --------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| iOS 앱 스와이프 강제 종료         | BGTask 미실행 → 오늘의 안부 확인 메시지 로컬 알림(10:00)으로 사용자에게 앱 실행 유도 → 무시 시 서버가 미수신 감지 → 보호자에게 경고                                                                                            
...
- **활성화** (`GuardianDashboardController.enableSubjectFeature()`):
  ...
  4. 첫 heartbeat 즉시 전송 (`HeartbeatService().execute()`) — 이후 SafetyCode `onInit`의 자동 재전송은 `HeartbeatService` 내부 `heartbeat_in_flight`(30s TTL mutex) + `lastScheduledKey`(성공 마커) + `isReportedToday=true`로 중복 전송이 구조적으로 차단됨
...
[1차: WorkManager(Android 2계층) / BGTaskScheduler(iOS) — 매일 예약 시각 (기본 09:30)]
    ...
    │    · lastScheduledKey(성공 마커) 검사 — 이미 성공한 스케줄이면 스킵
    │    · heartbeat_in_flight(30s TTL mutex) 검사 — 다른 isolate 전송 중이면 스킵
    │    · mutex 획득(saveHeartbeatInFlight) 후 센서/전송 진행, finally에서 clear
    │    · 과거 선점 save 방식은 Worker가 중도 종료될 때 ghost state를 남겨 2차 안전망을
    │      영구 차단하는 버그가 있었으므로, 성공 마커(lastScheduledKey)와 mutex 락
    │      (heartbeat_in_flight)의 역할을 분리함
    │
    ├─ 센서 스냅샷 (sensors_plus)
    ...
    │   │          iOS: BGTask 재예약
    │   │       ※ finally에서 heartbeat_in_flight clear (mutex lock 해제)
    │   └─ 미연결:
