...
| 안전망 (3차)          | (a) **periodic 15분 폴링** (WorkManager)이 1차 one-off 누락 시 최대 15분 내 백업 발화 (b) **앱 열기 자동 전송**(2차) (c) **일일 로컬 안전망 알림** (heartbeat 예약 시각 **+ 3시간**, 매일 반복) — worker 자체가 OEM/사용자에 의해 cancel되어 (a)(b) 모두 미동작인 LAST-RESORT를 메우는 사용자 유도 알림 | **오늘의 안부 확인 메시지 로컬 알림** (heartbeat 예약 시각 정시, 매일 반복) — BGTask 미사용으로 PRIMARY 트리거 |
| 앱 강제 종료 시       | WorkManager는 OS가 재스케줄링하므로 **정상 동작** (단, 정확한 시각 보장 불가). worker 영구 cancel 시에는 +3h 로컬 알림이 사용자 유도 | BGTask 미실행 → 정시 로컬 알림으로 사용자 유도                                            |
...
| 로컬 안전망 알림 시각   | **iOS**: 18:00 (heartbeat 시각 정시) / **Android**: 21:00 (heartbeat 시각 + 3시간) |
| 설정 변경               | 대상자 본인만 변경 가능                                                          |
...
| 3차 (안전망) | 일일 로컬 안부 확인 안전망 알림 (`LocalAlarmService`) | **iOS**: heartbeat 시각 정시 / **Android**: heartbeat 시각 + 3시간 (매일 반복, daily repeat via `matchDateTimeComponents.time`) | **iOS**: BGTask 미사용이므로 사실상 PRIMARY 트리거 — 사용자가 알림을 탭하면 앱이 열리며 `onInit`/`onResumed`에서 자동 전송. **Android**: WorkManager one-off + periodic 15분 + 앱 열기(2차) 모두 실패해 worker 자체가 OEM/사용자에 의해 cancel된 시나리오까지 메우는 LAST-RESORT 사용자 유도 알림. 자정 넘기면 자연 롤오버(예: 22:30 → 다음 날 01:30). 로컬 알림 탭 라우팅: iOS `gs_deadman` / Android `safety_net`·`send_failed` 모두 **safety_home으로 이동**(`_routeToSafetyHome`, 역할 인식; kill 런치는 splash). heartbeat 성공 시 `_onHeartbeatSent`가 `schedule(forceNextDay: true)` → `cancel(_alarmId)`로 예약 + 표시 중 알림 모두 제거 + 내일자 재예약 |
...
[3차 (안전망): 일일 로컬 안부 확인 알림 — 1차·2차 모두 실패 시]
    │
    ├─ iOS: heartbeat 시각 정시에 매일 반복 (BGTask 미사용으로 PRIMARY 트리거)
    ├─ Android: heartbeat 시각 + 3시간에 매일 반복 (LAST-RESORT, 자정 넘기면 자연 롤오버)
    │   ┌──────────────────────────────────────┐
    │   │ 💗 안부 확인이 필요합니다              │
    │   │                                      │
    │   │ 이 메시지 알림을 한 번 터치해 주세요.   │
    │   └──────────────────────────────────────┘
    │   ※ 기본: heartbeat 18:00 → iOS 알림 18:00 / Android 알림 21:00
    │   ※ repeats: true → 사용자가 앱을 열 때까지 매일 같은 시각에 반복
    │   ※ payload 분리: iOS `gs_deadman` / Android `safety_net`·`send_failed` 모두 safety_home으로 이동(`_routeToSafetyHome`)
    │
    └─ 사용자가 알림 탭 → 앱 포그라운드 전환 → 홈 컨트롤러 onInit/onResumed가 자동 전송

[정상 동작 시]
    매일 18:00 WorkManager/BGTask 실행 → heartbeat 성공 → _onHeartbeatSent가
    schedule(forceNextDay: true)로 cancel + 내일자 재예약 → 안전망 알림이 실제로
    표시될 일 없음 (Android 21:00 / iOS 18:00 모두 발화 전 cancel)
...
[비정상 시 — Android worker 영구 cancel + 앱 미사용]
    당일 21:00 → +3h 안전망 알림 표시 (worker 자체가 죽어 send_failed도 안 뜨는 상황)
...
- 보호 대상자 모드 선택 → 서버 등록 완료 시점에:
  ...
  2. 첫 heartbeat 즉시 전송 (HeartbeatService가 `_onHeartbeatSent`에서 일일 로컬 안전망 알림도 자동 등록)
...
[보호 대상자 모드 최초 등록 시]
    ...
    └─ 첫 heartbeat 성공 → _onHeartbeatSent가 schedule(forceNextDay: true)로
       일일 안전망 알림 등록 (iOS 다음 날 18:00 / Android 다음 날 21:00, 매일 반복)
...
- **Android Doze/OEM 절전 모드**: 스와이프 종료 + 화면 꺼짐 상태에서 one-off 태스크가 지연/미실행될 수 있음. periodic 15분 폴링이 최대 15분 내 백업 발화로 대부분 복구하며, 그래도 실패 시 앱 열기 자동 전송(2차)이 받아낸다. worker 자체가 OEM/사용자에 의해 영구 cancel된 LAST-RESORT는 +3h 일일 로컬 안전망 알림이 사용자에게 앱 실행을 유도. 사전 안내 다이얼로그는 두지 않고 사용자가 OEM 정책에 따라 직접 조정하도록 맡긴다 (Google Play 정책상 `REQUEST_IGNORE_BATTERY_OPTIMIZATIONS` 권한 미사용).
...
| 일일 로컬 안전망 알림 터치 (iOS 정시 / Android +3h)        | `main()` → **safety_home으로 이동** + 홈 화면 `onInit`/`onResumed`에서 예약시각 경과 + 미전송 시 자동 전송(알림 자체는 heartbeat 전송 안 함). iOS `gs_deadman` / Android `safety_net`·`send_failed` 모두 `_routeToSafetyHome` 적용 | 전체 화면 |
...
| **로컬 알림** — 일일 안전망/전송 실패(`gs_deadman`/`safety_net`/`send_failed`) | **safety_home** | iOS·Android·모든 앱 상태 동일. 미전송 heartbeat 자동 재전송 + 안내 다이얼로그는 홈/대시보드 컨트롤러 onResumed가 처리 |
...
| Android worker 영구 cancel + 앱 미사용 | WorkManager one-off + periodic 15분 모두 실행 안 됨 → +3h 일일 안전망 알림(기본 21:00)이 사용자에게 앱 실행 유도 → 앱 진입 시 2차 안전망이 자동 전송 + WorkManager 재등록으로 정상 사이클 복구                                                                              |
...
| `flutter_local_notifications` | 로컬 알림 예약/취소 | 일일 로컬 안부 확인 안전망 알림 (iOS 정시 / Android +3h) + Android retry 실패 안내 알림 + 배터리/네트워크 안내 알림 |
...
- **앱 재진입 시 스케줄 동기화** (`_scheduleHeartbeatIfGS()`): Dashboard `onInit`에서 G+S 여부 확인 후 서버 `GET /api/v1/devices/me`로 heartbeat 스케줄 재동기화 + **WorkManager만 재등록**. 로컬 안전망 알림은 재예약하지 않는다(S `_syncScheduleFromServer`와 동일) — 안전망 알림 재예약은 `HeartbeatService._onHeartbeatSent`가 `forceNextDay`로 단일 소유하며, onInit에서 `forceNextDay` 없이 재호출하면 이미 오늘 전송돼 내일로 옮겨둔 알림을 (heartbeat+3h가 오늘 미래일 때) 당일로 되돌리는 버그가 발생한다(재설치 후 미래 시각 변경 시 안전망 알림이 당일 발화하던 결함 — 과거 G+S onInit에만 누락)
...
[3차 (안전망): 일일 로컬 안부 확인 알림 — 1차·2차 모두 실패 시]
    │
    ├─ iOS: heartbeat 시각 정시에 매일 반복 로컬 알림 ("💗 안부 확인이 필요합니다 / 이 메시지 알림을 한 번 터치해 주세요.")
    ├─ Android: heartbeat 시각 + 3시간에 매일 반복 (LAST-RESORT, 자정 넘기면 자연 롤오버)
    ├─ 사용자가 알림 탭 → 앱 포그라운드 전환만 (알림 자체에서 heartbeat 전송 안 함)
    │   → 홈 화면 onInit/onResumed에서 예약시각 경과 + 미전송 시 자동 전송
    ├─ 무시 시 매일 같은 시각에 계속 반복
    └─ heartbeat 성공 시 _onHeartbeatSent → schedule(forceNextDay: true) → cancel(_alarmId)으로
