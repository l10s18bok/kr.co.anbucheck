...
- **Android Doze/OEM 절전 모드**: 스와이프 종료 + 화면 꺼짐 상태에서 one-off 태스크가 지연/미실행될 수 있음. periodic 1시간 폴링이 최대 1시간 내 백업 발화로 대부분 복구하며, 그래도 실패 시 앱 열기 자동 전송(2차)이 최종 안전망. 사전 안내 다이얼로그는 두지 않고 사용자가 OEM 정책에 따라 직접 조정하도록 맡긴다 (Google Play 정책상 `REQUEST_IGNORE_BATTERY_OPTIMIZATIONS` 권한 미사용).
...
- 배터리 최적화 제외 권한(`REQUEST_IGNORE_BATTERY_OPTIMIZATIONS`)은 매니페스트에 선언하지 않으며 별도 사전 안내 다이얼로그도 두지 않음 (Doze 누락은 §2.2 "3계층 Heartbeat 실행 구조"의 periodic 1시간 폴링 + 앱 열기 자동 전송으로 보완)
...
- **SubjectHomeController 핵심 동작 (`lib/app/modules/safety_home/controllers/`, `SafetyHomeBaseController` 상속):**
  ...
  - **첫 설치 시각 가드 우회**: `lastHeartbeatDate.isEmpty`(전송 이력 없음)이면 `isScheduleInFuture`/`isScheduleTooOld` 검사를 건너뛰고 즉시 전송 — Google Fit 구독 생성 + last_seen baseline + 등록→heartbeat 파이프라인 검증을 한 번에 처리해 D0(설치 당일) 갭 해소. 첫 전송 후 `lastHeartbeatDate`가 박히면 이후 진입에서는 정상 가드가 다시 적용된다
