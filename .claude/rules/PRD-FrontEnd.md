...
[1차: WorkManager/BGTaskScheduler — 백그라운드 실행]
    ...
    │            내일자 재등록 (idempotent, periodic의 self-cancel은 currently
    │            실행 중인 task는 유지하고 다음 예약만 취소하므로 안전)
    │       ※ worker 콜백 끝의 동일 schedule() 호출은 안전망으로 유지
    │          (서비스 호출이 어떤 이유로 누락되어도 worker 경로 보장)
    ...
    │        send_failed 알림 제거
    │      ※ worker 콜백 끝에도 동일 schedule() 호출 (안전망, idempotent)
    └─ 실행 후 자동 종료
