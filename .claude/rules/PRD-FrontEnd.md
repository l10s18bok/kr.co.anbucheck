...
**활동 지표: 걸음수(primary) + 가속도/자이로/지자기(secondary) 결합 분석**
...
[heartbeat 실행 시]
    ...
    └─ 2단계: 걸음수 변화 없을 때만 (steps_delta = 0)
        │
        ├─ 가속도계(accelerometer) 스냅샷 조회 (sensors_plus) — 기울기(pitch/roll) 변화 감지
        ├─ 자이로스코프(gyroscope) 스냅샷 조회 (sensors_plus) — 회전 속도 변화 감지
        ├─ 지자기(magnetometer) 스냅샷 조회 (sensors_plus)   — 수평 회전(yaw) 변화 감지
        │   ※ 가속도/자이로 사각지대: 책상 위에서 폰을 수평 회전(0도→90도)해도
        │     가속도·자이로값은 불변 → 지자기로만 감지 가능
        │   ※ 미지원 기기는 지자기 조건을 판정에서 제외 (500ms 타임아웃 후 null 처리)
        ├─ 이전 스냅샷과 비교
        │   · 가속도 변화량 = √((Δx)² + (Δy)² + (Δz)²)
        │   · 자이로 변화량 = √((Δx)² + (Δy)² + (Δz)²)
        │   · 지자기 변화량 = √((Δx)² + (Δy)² + (Δz)²)
        │
        ├─ 의심 조건 (모든 조건 충족 시):
        │   · 가속도 변화량 < 5.0 m/s²   (~30도 기울기 변화 미만)
        │   · 자이로 변화량 < 0.3 rad/s
        │   · 지자기 변화량 < 20.0 μT    (~30도 수평 회전 변화 미만)
        │   → suspicious = true
        │
        └─ 정상 조건 (하나라도 충족 시):
            · 가속도 변화량 ≥ 5.0 m/s²
            · 자이로 변화량 ≥ 0.3 rad/s
            · 지자기 변화량 ≥ 20.0 μT   (수평 회전 ~30도 이상)
            → suspicious = false
```
...
**센서 미지원 기기 대응:**
- 걸음수 권한 거부 시 → 가속도/자이로/지자기 단독으로 판별
- 지자기 센서 미지원 시 → 가속도/자이로만으로 판별 (지자기 조건 자동 제외)
...
| WorkManager/BGTask (매일 예약 시각)              | 센서 스냅샷(가속도+자이로+지자기) 조회 → heartbeat 전송 → 다음날부터 해당 시각 매일 재예약(전송당일 예약x) | 없음      |
...
### 3.3 suspicious 판정 (걸음수 primary + 가속도/자이로/지자기 secondary)

**핵심:** 걸음수 변화가 있으면 즉시 정상 판정. 걸음수 0일 때만 가속도/자이로/지자기로 보완 판정.

**지자기 추가 배경:** 가속도/자이로는 수평 회전(yaw, 책상 위에서 폰을 돌리는 동작)을 감지하지 못하는 사각지대가 있다. 충전 거치대에 매일 같은 각도로 올려놓는 대상자가 steps_delta=0인 경우 가속도/자이로만으로는 영구적으로 suspicious=true 판정이 날 수 있어, 지자기 센서로 수평 회전 변화를 보완 감지한다.
...
[heartbeat 수집 시 활동 판정]
    ...
    2단계: steps_delta = 0 일 때
      의심 조건 (모든 조건 충족 시 suspicious = true):
        · accel_delta < 5.0 m/s²   (가속도 변화 없음 — ~30도 기울기 변화 미만)
        · gyro_delta < 0.3 rad/s   (자이로 변화 없음)
        · mag_delta < 20.0 μT      (지자기 변화 없음 — ~30도 수평 회전 미만)

      정상 조건 (하나라도 충족 시 suspicious = false):
        · accel_delta ≥ 5.0 m/s²
        · gyro_delta ≥ 0.3 rad/s
        · mag_delta ≥ 20.0 μT     (수평 회전 ~30도 이상 감지)

    ※ 걸음수 권한 거부 시 → 2단계(가속도/자이로/지자기)만으로 판정
    ※ 지자기 센서 미지원 기기 → mag_delta 조건 자동 제외 (가속도/자이로만으로 판정)
...
| `sensors_plus` | 가속도/자이로/지자기 센서 조회 | **secondary 활동 지표** (걸음수 0일 때 보완). 가속도/자이로로 기울기·회전 감지, 지자기로 수평 회전(yaw) 사각지대 보완. 권한 불필요 |
...
[1차: WorkManager(Android 2계층) / BGTaskScheduler(iOS) — 매일 예약 시각 (기본 18:00)]
    ...
    │      — 성공 → 센서/전송 진행, finally에서 release(scheduledKey)
