...
| 트리거                | `registerOneOffTask()` — 정확한 시각 지정 (단일 계층, periodic 안전망은 one-off와 동시 실행되는 중복 전송 race 때문에 드롭) | `registerProcessingTask()` → BGProcessingTask (earliestBeginDate 존중). ⚠️ `registerOneOffTask()`는 iOS에서 `beginBackgroundTask`를 사용하여 즉시 실행되므로 사용 금지 (flutter_workmanager PR #511) |
...
| 안전망 (3차)          | 없음 — 앱 열기 자동 전송(2차)이 유일한 안전망. Doze/OEM 배터리 절약으로 one-off 미실행 시 사용자가 앱을 열 때까지 복구 불가 | **로컬 반복 알림 데드맨 스위치** (heartbeat 예약 시각, 매일 반복) — BGTask 실패 시 사용자에게 앱 실행 유도  |
...
| 계층 | 방식 | 실행 조건 | 특성 |
|------|------|-----------|------|
| 1차 | WorkManager (Android) / BGTaskScheduler (iOS 미사용) | 예약 시각 도래 (one-off 단일 계층) | 백그라운드 자동 실행, 사용자 인지 불가, 당일 중복 전송 방지. periodic 안전망은 one-off와의 race 때문에 드롭 |
| 2차 | 앱 열기/복귀 자동 전송 | 예약 시각 경과 + 당일 미전송 | 포그라운드 진입 시 즉시 전송 (Android의 유일한 안전망) |
| 3차 (iOS) | 로컬 알림 데드맨 스위치 | heartbeat 시각 + 30분 (매일 반복, iOS 전용) | 1차·2차 모두 실패 시 사용자에게 앱 실행 유도. Android는 데드맨 알림 없음 — 2차가 유일한 안전망 |
...
[1차: WorkManager/BGTaskScheduler — 백그라운드 실행]
    ...
    │       ※ periodic 안전망은 one-off와 동시 실행되는 중복 전송 race 때문에 드롭됨
    │
    ��─ iOS:
...
- **Android Doze/OEM 절전 모드**: 스와이프 종료 + 화면 꺼짐 상태에서 one-off 태스크가 지연/미실행 가능. 현재는 사용자가 앱을 열 때까지 복구 불가 — Splash의 "배터리 제한없음" 안내 다이얼로그(9.0.1)가 유일한 예방책
...
- **활성화** (`enableSubjectFeature()`):
  ...
  4. 첫 heartbeat 즉시 전송
  5. 성공 시 `goToSafetyCode()` 호출 → 안전코드 페이지(SubjectHome)로 자동 이동
  ※ 활동 인식 권한은 최초 권한 화면(9.0)에서 이미 요청되었으므로 별도 요청 없음
  ※ iOS는 활성화 전 경고 다이얼로그(`_showEnableConfirmIOS`)를 반드시 표시 — "매일 정해진 시각의 안부 푸시 알림을 탭하거나 그 전후로 앱을 열어야 안부가 전송됨" 안내 후 `이해했습니다, 활성화` 버튼으로만 확정
...
- **전송 조건**: `isReportedToday == false` 하나뿐. 시각 경과 여부는 확인하지 않음.
  ...
  - `SubjectHomeController._checkAndSendHeartbeat` / `GuardianSettingsController._checkAndSendHeartbeat` 모두 동일 규칙
- **데드맨 알림 시각**: iOS G+S는 heartbeat 예약 시각 +30분이 아니라 **예약 시각과 동일** (`LocalAlarmService.schedule`에서 오프셋 제거)
- **데드맨 알림 탭 라우팅** (`fcm_service._handleNotificationTap`): payload `gs_deadman` 수신 시 `Get.offAllNamed(guardianDashboard)` + `Get.toNamed(subjectHome)` — 보호자 대시보드 스택 위에 대상자 홈을 push하여, 뒤로가기 시 자연스럽게 대시보드로 복귀. 단, 이미 `SubjectHome`에 있는 경우(`Get.currentRoute == subjectHome`) 스택을 유지하여 기존 arguments(`deviceData`)와 뒤로가기 버튼을 보존 — `onResumed`가 자동으로 미전송 heartbeat 전송
...
```
[1차: WorkManager/BGTaskScheduler 백그라운드 실행 — 매일 예약 시각 (기본 09:30)]
    │
    ├─ 역할 확인 (대상자 모드만 실행)
    ├─ 당일 이미 전송 여부 확인 (lastHeartbeatDate == 오늘 → 스킵)
    │
    ├─ 센서 스냅샷 (sensors_plus)
    ...
    │   │   └─ 전송 성공 시:
    │   │       ├─ lastHeartbeatDate 갱신 (오늘 날짜)
    │   │       ├─ 데드맨 스위치 반복 알림 취소+재등록
    │   │       └─ 다음 날 동일 시각으로 WorkManager 재예약
    │   └─ 미연결:
