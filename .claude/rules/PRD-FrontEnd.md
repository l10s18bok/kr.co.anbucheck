...
| 안전망 (3차)          | WorkManager periodic(1시간 주기)이 안전망 역할 — 별도 로컬 알림 불필요   | **로컬 반복 알림 데드맨 스위치** (heartbeat 시각 + 30분, 매일 반복) — BGTask 실패 시 사용자에게 앱 실행 유도  |
...
| 계층 | 방식 | 실행 조건 | 특성 |
|------|------|-----------|------|
| 1차 | WorkManager / BGTaskScheduler + Background Fetch | 예약 시각 도래 (one-off) + OS 주기적 실행 (periodic) | 백그라운드 자동 실행, 사용자 인지 불가, 당일 중복 전송 방지 |
| 2차 | 앱 열기/복귀 자동 전송 | 예약 시각 경과 + 당일 미전송 | 포그라운드 진입 시 즉시 전송 |
| 3차 (iOS) | 로컬 알림 데드맨 스위치 | heartbeat 시각 + 30분 (매일 반복, iOS 전용) | 1차·2차 모두 실패 시 사용자에게 앱 실행 유도. Android는 WorkManager periodic이 안전망 역할 |
...
[3차 (iOS 전용): 로컬 알림 데드맨 스위치 — 1차·2차 모두 실패 시]
    ...
    │   ※ Android는 WorkManager periodic(1시간 주기)이 안전망 역할 → 데드맨 알림 불필요
    │
