...
| 안전망 (3차)          | **로컬 반복 알림 데드맨 스위치** (heartbeat 시각 + 30분, 매일 반복) — WorkManager 실패 시 사용자에게 앱 실행 유도   | **로컬 반복 알림 데드맨 스위치** (heartbeat 시각 + 30분, 매일 반복) — BGTask 실패 시 사용자에게 앱 실행 유도  |
...
```
- 보호 대상자가 직접 변경: 서버에 반영 + WorkManager/BGTask 재예약 + 로컬 알림 시각 갱신
...
[3차: 로컬 알림 데드맨 스위치 — 1차·2차 모두 실패 시]
    │
    ├─ heartbeat 시각 + 30분에 매일 반복 로컬 알림
    │   ┌──────────────────────────────────────┐
    │   │ 📱 안부 확인이 필요합니다              │
    │   │                                      │
...
[정상 동작 시]
    매일 09:30 WorkManager 실행 → heartbeat 성공 → 예약시각 매일 재예약
    → 데드맨 알림(10:00)이 실제로 표시될 일 없음

[비정상 시 (WorkManager/BGTask 미실행, 앱 미사용)]
    당일 10:00 → 데드맨 알림 표시
...
- 보호 대상자 모드 선택 → 서버 등록 완료 시점에:
  ...
  2. 데드맨 스위치 로컬 알림 등록 (heartbeat 시각 + 30분, repeats: true)
  3. 첫 heartbeat 즉시 전송
...
[보호 대상자 모드 최초 등록 시]
    ...
    ├─ 데드맨 스위치 매일 반복 알림 등록 (기본 10:00, repeats: true)
...
| 데드맨 스위치 로컬 알림 터치 (대상자)            | `main()` → 앱 포그라운드 전환만 (알림 자체에서 heartbeat 전송 안 함) → 홈 화면 `onInit`/`onResumed`에서 예약시각 경과 + 미전송 시 자동 전송 | 전체 화면 |
| suspicious 로컬 알림 터치 (대상자)               | `main()` → manual=true heartbeat 즉시 재전송 (이미 전송됐어도 무조건 재전송 — 사용자 생존 응답) | 전체 화면 |
...
```
[suspicious = true 판정 시 즉시]
    │
    ├─ 대상자 폰에 로컬 알림 즉시 표시 (서버 왕복 없음, 오프라인에서도 동작)
    │   ┌──────────────────────────────────────┐
    │   │ 💛 안부 확인                           │
    │   │                                      │
    │   │ 잘 지내고 계시죠?                      │
    ...
    │
    ├─ 대상자가 알림 터치
    │   → manual=true heartbeat 즉시 재전송 (이미 전송됐어도 무조건 재전송)
    │   → 센서 수집/비교 스킵 (stepsDelta=null, suspicious=false)
    │   → 버튼을 직접 눌렀다는 행위 자체가 활동 증거
    │   → 보호자에게 "수동 안부 확인" 알림
    │       ┌──────────────────────────────────────┐
    │       │ ✅ 수동 안부 확인                      │
    │       │                                      │
    ...
    │       └──────────────────────────────────────┘
    │
    └─ 대상자가 무시하면 → 다음 heartbeat 주기까지 대기
        → 보호자 알림은 suspicious_count 기반 에스컬레이션으로 별도 발송됨
          (1회→주의, 2회→경고, 3회+→긴급)
```
...
[3차: 데드맨 스위치 로컬 알림 — heartbeat 시각 + 30분 (1차·2차 실패 시)]
    │
    ├─ 매일 반복 로컬 알림 표시 ("안부 확인이 필요합니다. 이 메시지 알림을 한 번 터치해 주세요.")
    ├─ 사용자가 알림 탭 → 앱 포그라운드 전환만 (알림 자체에서 heartbeat 전송 안 함)
    │   → 홈 화면 onInit/onResumed에서 예약시각 경과 + 미전송 시 자동 전송
    └─ 무시 시 매일 같은 시각에 계속 반복

[suspicious 로컬 알림 — heartbeat 전송 시 suspicious=true 판정 시 즉시]
    │
    ├─ 로컬 알림 즉시 표시 ("잘 지내고 계시죠? 이 메시지 알림을 한 번 터치해 주세요.")
    ├─ 사용자가 알림 탭 → manual=true heartbeat 즉시 재전송 (이미 전송됐어도 무조건)
    │   → 센서 수집/비교 스킵, suspicious=false로 서버 전송 → 경고 해소
    └─ 무시 시 다음 heartbeat 주기까지 대기
