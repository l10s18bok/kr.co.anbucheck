...
/// 상세한 측정 의도는 `HeartbeatAlarmReceiver`(Kotlin) 주석에 있다.
///
...
class HeartbeatAlarm {
  static const _channel = MethodChannel('anbucheck/heartbeat_alarm');
  ...
  static Future<void> arm(int hour, int minute) async {
    ...
    try {
      ...
      debugPrint('[HeartbeatAlarm] armed $hour:$minute');
    } catch (e) {
      // 백그라운드 isolate에는 MainActivity가 없어 MissingPluginException이 정상이다.
      debugPrint('[HeartbeatAlarm] arm 스킵: $e');
    }
  }
